...
def test_empathy_evaluator(dummy_conversation):
    ...
    assert "empathy_progression_score" in res5 and isinstance(
        res5["empathy_progression_score"], float
    )
    assert "empathy_scores" in res5 and isinstance(res5["empathy_scores"], list)
