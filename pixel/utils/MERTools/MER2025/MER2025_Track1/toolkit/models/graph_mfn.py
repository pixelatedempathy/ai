...
class DynamicFusionGraph(nn.Module):
    def __init__(self, pattern_model, in_dimensions, out_dimension, efficacy_model):
        ...
        self.powerset = list(
            chain.from_iterable(
                combinations(range(self.num_modalities), r) for r in range(self.num_modalities + 1)
            )
        )[1:]

        # initializing the models inside the DFG
        self.input_shapes = {
            tuple([key]): value for key, value in zip(range(self.num_modalities), in_dimensions)
        }

        self.networks = {}
        self.total_input_efficacies = (
            0  # total_input_efficacies: for alpha list size = [batch_size, total_input_efficacies].
        )
        ...
        for key in self.powerset[self.num_modalities :]:
            ...
            final_model = nn.Sequential(
                *[
                    nn.Linear(self.input_shapes[key], list(pattern_copy.children())[0].in_features),
                    pattern_copy,
                ]
            ).cuda()
        ...
        self.t_in_dimension = (
            unimodal_dims + (2**self.num_modalities - (self.num_modalities) - 1) * out_dimension
        )
        pattern_copy = copy.deepcopy(pattern_model)
        ...
        self.t_network = nn.Sequential(
            *[
                nn.Linear(self.t_in_dimension, list(pattern_copy.children())[0].in_features),
                pattern_copy,
            ]
        ).cuda()
        ...
        self.efficacy_model = nn.Sequential(
            *[
                ...
                efficacy_model,
                nn.Linear(
                    list(efficacy_model.children())[-1].out_features, self.total_input_efficacies
                ),
            ]
        ).cuda()
    ...
    def fusion(self, in_modalities):
        ...
        efficacy_index = 0
        for key in self.powerset[self.num_modalities :]:
            small_power_set = list(
                chain.from_iterable(combinations(key, r) for r in range(len(key) + 1))
            )[1:-1]
            this_input = torch.cat(
                [
                    outputs[x] * efficacies[:, efficacy_index + y].view(-1, 1)
                    for x, y in zip(small_power_set, range(len(small_power_set)))
                ],
                dim=1,
            )
...
class Graph_MFN(nn.Module):
    def __init__(self, args):
        ...
        efficacy_model = nn.Sequential(
            nn.Linear(100, self.hidden_dim)
        ).cuda()  # Note : actually here inner_node_dim can change arbitrarily
    ...
    def forward(self, batch):
        ...
        for i in range(t):
            ...
            attended, _, efficacies = self.graph_mfn(
                [l_singleton_input, a_singleton_input, v_singleton_input]
            )
            ...
            gamma1 = torch.sigmoid(
                self.gamma1_fc2(self.gamma1_dropout(F.relu(self.gamma1_fc1(both))))
            )
            gamma2 = torch.sigmoid(
                self.gamma2_fc2(self.gamma2_dropout(F.relu(self.gamma2_fc1(both))))
            )
