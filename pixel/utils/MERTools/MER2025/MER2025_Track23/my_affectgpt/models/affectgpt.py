...
class AffectGPT(Blip2Base):
    ...
    def __init__(
        ...
        if llama_model_name in ["Baichuan2"]:
            ...
            self.llama_model = AutoModelForCausalLM.from_pretrained(
                config.PATH_TO_LLM[llama_model_name], torch_dtype=torch.float16
            )
        ...
        self.image_llama_proj = nn.Linear(
            self.visual_encoder.hidden_size, self.llama_model.config.hidden_size
        )
        ...
        if self.video_fusion_type == "qformer":
            self.video_frame_position_embedding = nn.Embedding(
                32, self.visual_encoder.hidden_size
            )  # [32, featdim]
        ...
        self.max_hidden_size = max(
            self.acoustic_encoder.hidden_size, self.visual_encoder.hidden_size
        )
    ...
    def encode_image_token(self, image, raw_image):
        ...
        with self.maybe_autocast():
            ...
            # case2: 输入维度为 [b, t, 768]
                ...
                frame_hidden_state = frame_hidden_state.expand(
                    -1, self.num_image_query_token, -1
                )  # [b, t=img_token_num, featdim]
    ...
    def encode_image_mean(self, image, raw_image):
        ...
        with self.maybe_autocast():
            ...
            inputs_llama = torch.unsqueeze(inputs_llama, 1).expand(
                -1, self.num_image_query_token, -1
            )  # [b, 16, llmdim]
    ...
    def encode_image_merge(self, image, raw_image):
        if self.image_fusion_type == "token":
            image_hiddens, image_llms = self.encode_image_token(
                image, raw_image
            )  # image: [b c 1 h w] -> [b, 1,  4096]; raw_images: b * [Image.Image]
    ...
    def encode_video_qformer(self, video, raw_video):
        ...
        with self.maybe_autocast():
            ...
            store_hidden_state = ""
            if len(frame_hidden_state.size()) == 4:
                store_hidden_state = einops.rearrange(
                    frame_hidden_state, "b t q h -> b (t q) h", b=batch_size, t=time_length
                )
            elif len(frame_hidden_state.size()) == 3:
            ...
            if len(frame_hidden_state.size()) == 4:
                ...
                frame_hidden_state = (
                    frame_position_embeddings + frame_hidden_state
                )  # [b, t, 32, 768]
                frame_hidden_state = einops.rearrange(
                    frame_hidden_state, "b t q h -> b (t q) h", b=batch_size, t=time_length
                )  # [b, (t, 32), 768]
    ...
    def encode_video_mean(self, video, raw_video):
        ...
        with self.maybe_autocast():
            ...
            if len(hidden_state.size()) == 4:
                store_hidden_state = einops.rearrange(
                    hidden_state, "b t q h -> b (t q) h", b=batch_size, t=time_length
                )
            elif len(hidden_state.size()) == 3:
            ...
            inputs_llama = torch.unsqueeze(inputs_llama, 1).expand(
                -1, self.num_video_query_token, -1
            )  # [b, 16, llmdim]
    ...
    def encode_video_attention(self, video, raw_video):
        ...
        with self.maybe_autocast():
            ...
            hidden_state = einops.rearrange(
                hidden_state, "b t h -> b h t", b=batch_size, t=time_length
            )  # [b, h, t]
    ...
    def encode_video_merge(self, video, raw_video):
        if self.video_fusion_type == "qformer":
            frame_hiddens, frame_llms = self.encode_video_qformer(
                video, raw_video
            )  # frame: [b c t h w] -> [b, 32, 4096]
        elif self.video_fusion_type == "attention":
    ...
    def encode_audio_mean(self, audio, raw_audio):
        ...
        with self.maybe_autocast():
            ...
            inputs_llama = torch.unsqueeze(inputs_llama, 1).expand(
                -1, self.num_audio_query_token, -1
            )  # [b, 16, llmdim]
    ...
    def encode_audio_merge(self, audio, raw_audio):
        if self.audio_fusion_type == "qformer":
            audio_hiddens, audio_llms = self.encode_audio_qformer(
                audio, raw_audio
            )  # audio: [b t c h w] -> [b, 8,  4096]
        elif self.audio_fusion_type == "attention":
    ...
    def encode_multi_qformer(self, video_hidden_state, audio_hidden_state):
        ...
        with self.maybe_autocast():
            ...
            multi_hidden_state = torch.concat(
                [video_hidden_state, audio_hidden_state], axis=1
            )  # [b, ta+tv, maxdim]
            ...
            position_ids = torch.arange(time_length, dtype=torch.long, device=device)
            position_ids = position_ids.unsqueeze(0).expand(batch_size, -1)
            multi_position_embeddings = self.multi_position_embedding(
                position_ids
            )  # [b, ta+tv, maxdim]
            multi_hidden_state = (
                multi_hidden_state + multi_position_embeddings
            )  # [b, ta+tv, maxdim]
    ...
    def encode_multi_attention(self, video_hidden_state, audio_hidden_state):
        ...
        with self.maybe_autocast():
            ...
            multi_hidden2 = torch.stack(
                [video_hidden_state, audio_hidden_state], dim=2
            )  # [b, maxdim, 2]
            ...
            inputs_llama = torch.unsqueeze(inputs_llama, 1).expand(
                -1, self.num_multi_query_token, -1
            )  # [b, 16, llmdim]
    ...
    def encode_multi_merge(self, video_hidden_state, audio_hidden_state):
        if self.multi_fusion_type == "qformer":
            multi_hiddens, multi_llms = self.encode_multi_qformer(
                video_hidden_state, audio_hidden_state
            )
        elif self.multi_fusion_type == "attention":
            multi_hiddens, multi_llms = self.encode_multi_attention(
                video_hidden_state, audio_hidden_state
            )
    ...
    def forward(self, samples):
        ...
        if "frames" in samples:
            frame_hiddens, frame_llms = self.encode_video_merge(
                samples["frames"], samples["raw_frames"]
            )  # frame: [b c t h w] -> [b, 32, 4096]
        if "faces" in samples:
            # print (samples['faces'].shape)
            face_hiddens, face_llms = self.encode_video_merge(
                samples["faces"], samples["raw_faces"]
            )  # face:  [b c t h w] -> [b, 32, 4096]
        if "audios" in samples:
            audio_hiddens, audio_llms = self.encode_audio_merge(
                samples["audios"], samples["raw_audios"]
            )
        if "images" in samples:
            image_hiddens, image_llms = self.encode_image_merge(
                samples["images"], samples["raw_images"]
            )
        if (
            samples["input_ids"][0] == self.MULTI_PATCH_TOKEN_ID
        ).sum() != 0:  # 这是时候才需要 multi
        ...
        temp_input_embedding = self.llama_model.model.model.embed_tokens(
            temp_input_ids
        )  # 嵌套 LoRA 之后，会在 model 外面再包一层
            ...
            for patch_token_id, query_token_number, embeds in [
                ...
                if (cur_input_ids == patch_token_id).sum() != 0:
                    ...
                    if (cur_input_ids == patch_token_id).sum() != query_token_number:
                        raise ValueError(
                            "The number of audio patch tokens should be the same as the number of audio patches."
                        )
