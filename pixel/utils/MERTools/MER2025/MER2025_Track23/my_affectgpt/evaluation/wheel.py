...
def generate_different_format_for_words():
    ...
    if not os.path.exists(save_root_format1):
        os.makedirs(save_root_format1)
    if not os.path.exists(save_root_format2):
        os.makedirs(save_root_format2)
...
def get_raw2format(format_root):
    ...
    for file_path in glob.glob(format_root + "/*"):
        ...
        for format in formats:
            if len(format.split()) != len(raw.split()):
                continue
            if len(format.split("-")) != len(raw.split("-")):
                continue
...
def func_get_wheel_cluster(wheel="wheel1", level="level1"):
    ...
    # 2. 全部聚类到level2
        ...
        for level1 in emotion_wheel:
            wheel_map[level1] = sorted(emotion_wheel[level1])[
                0
            ]  # level1 映射到一个固定的 level2 上
            for level2 in emotion_wheel[level1]:
...
def wheel_metric_calculation(
    ...
    for metric in candidate_metrics:
        precision, recall = calculate_openset_overlap_rate(
            gt_root=gt_root,
            gt_csv=gt_csv,
            name2gt=name2gt,
