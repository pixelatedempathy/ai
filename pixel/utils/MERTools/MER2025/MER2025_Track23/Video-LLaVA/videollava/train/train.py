...
class DataArguments:
    ...
    num_frames: int = 8
    # Dynamically set attributes for processors to avoid Pylance/mypy errors
    image_processor: Optional[object] = None
    video_processor: Optional[object] = None
...
def safe_save_model_for_hf_trainer(trainer: HFTrainer, output_dir: str):
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        weight_to_save = get_mm_adapter_state_maybe_zero_3(
            trainer.model.named_parameters(), keys_to_match  # type: ignore[attr-defined]
        )  # type: ignore[union-attr]
        if hasattr(trainer.model, "config") and hasattr(trainer.model.config, "save_pretrained"):  # type: ignore[union-attr]
            trainer.model.config.save_pretrained(output_dir)  # type: ignore[union-attr]
        # type: ignore[union-attr]

        current_folder = output_dir.split("/")[-1]
        parent_folder = os.path.dirname(output_dir)
        if getattr(trainer.args, "local_rank", None) == 0 or getattr(trainer.args, "local_rank", None) == -1:
    ...
    if getattr(trainer, "deepspeed", False):
        torch.cuda.synchronize()
        trainer.save_model(output_dir)
        return

    state_dict = trainer.model.state_dict()  # type: ignore[union-attr]
    if getattr(trainer.args, "should_save", False):
        cpu_state_dict = {key: value.cpu() for key, value in state_dict.items()}
        del state_dict
        trainer._save(output_dir, state_dict=cpu_state_dict)  # noqa  # type: ignore[attr-defined]
...
def smart_tokenizer_and_embedding_resize(
    ...
    if num_new_tokens > 0:
        input_embeddings = model.get_input_embeddings().weight.data  # type: ignore[attr-defined]
        output_embeddings = model.get_output_embeddings().weight.data  # type: ignore[attr-defined]

        input_embeddings_avg = input_embeddings[:-num_new_tokens].mean(dim=0, keepdim=True)  # type: ignore[index]
        output_embeddings_avg = output_embeddings[:-num_new_tokens].mean(dim=0, keepdim=True)  # type: ignore[index]

        input_embeddings[-num_new_tokens:] = input_embeddings_avg  # type: ignore[index, assignment]
        output_embeddings[-num_new_tokens:] = output_embeddings_avg  # type: ignore[index, assignment]
...
def _tokenize_fn(strings: Sequence[str], tokenizer: PreTrainedTokenizer) -> Dict:
    ...
    labels_lens = [tokenized.input_ids.ne(tokenizer.pad_token_id).sum().item() for tokenized in tokenized_list]
    return dict(
        input_ids=input_ids,
        labels=labels,
...
def preprocess_multimodal(sources: Sequence, data_args: DataArguments):
    is_multimodal = getattr(data_args, "is_multimodal", False)
    ...
    for source in sources:
        for sentence in source:
            # ======================================================================================================
            if isinstance(sentence, dict) and (
                sentence.get("value", "").startswith(DEFAULT_IMAGE_TOKEN)
                or sentence.get("value", "").startswith(DEFAULT_VIDEO_TOKEN)
            ):
                # <video><video><image><image>\nxxxxxxxxxxxxx  # must <video> first
                # <image>\nxxxxxxxxxxxxx -> <image>\nxxxxxxxxxxxxx
                # <video>\nxxxxxxxxxxxxx -> <video>\nxxxxxxxxxxxxx

                if "mmtag" in getattr(conversation_lib.default_conversation, "version", ""):
            ...
            replace_token, vid_replace_token = (
                DEFAULT_IMAGE_TOKEN,
                DEFAULT_IMAGE_TOKEN * getattr(data_args, "num_frames", 8),
            )
            if getattr(data_args, "mm_use_im_start_end", False):
...
def preprocess_llama_2(
    sources, tokenizer: PreTrainedTokenizer, has_image: bool = False
) -> Dict:
    ...
    if has_image:
        input_ids = torch.stack(
            [
                ...
                if isinstance(tokenizer_image_token(prompt, tokenizer, return_tensors="pt"), torch.Tensor)
            ],
            dim=0,
    ...
    for conversation, target in zip(conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())  # type: ignore[attr-defined]
...
def preprocess_plain(
    ...
    for target, source in zip(targets, sources):
        ...
        try:
            target[:tokenized_len] = IGNORE_INDEX  # type: ignore[index]
        except Exception:
            pass
...
def preprocess(
    sources: Sequence, tokenizer: PreTrainedTokenizer, has_image: bool = False
) -> Dict:
    """
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        self, data_path, tokenizer: PreTrainedTokenizer, data_args: DataArguments
    ):
        super().__init__()
        # ================================================
        list_data_dict = []
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if "image" in sources[0] and "video" not in sources[0]:
                ...
                image_processor = getattr(self.data_args, "image_processor", None)
                image_file = image_file if isinstance(image_file, list) else [image_file]
                image_file = [f for f in image_file if isinstance(f, str)]
                image_file = order_pick_k(image_file, MAX_IMAGE_LENGTH)
                # print(f"total {len(self.list_data_dict[i]['image'])} now {len(image_file)}")
                image = [
                    Image.open(os.path.join(image_folder, file)).convert("RGB")  # type: ignore[arg-type]
                    for file in image_file if file is not None and isinstance(image_folder, str)
                ]
                if self.data_args.image_aspect_ratio == "pad" and image_processor is not None:
                    image = [
                        expand2square(i, tuple(int(x * 255) for x in getattr(image_processor, "image_mean", [0.5, 0.5, 0.5])))
                        for i in image
                    ]
                    image = [
                ...
                video_processor = getattr(self.data_args, "video_processor", None)
                video_file = video_file if isinstance(video_file, list) else [video_file]
                video_file = [f for f in video_file if isinstance(f, str)]
                video_file = order_pick_k(video_file, MAX_VIDEO_LENGTH)
                video = [os.path.join(video_folder, file) for file in video_file if file is not None and isinstance(video_folder, str)]  # type: ignore[arg-type]
                image = [
                    video_processor(i, return_tensors="pt")["pixel_values"][0] for i in video
                ] if video_processor is not None else []
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids, batch_first=True, padding_value=int(self.tokenizer.pad_token_id)
        )
        ...
        batch = dict(
            input_ids=input_ids,
            labels=labels,
            attention_mask=(input_ids != int(self.tokenizer.pad_token_id)),  # type: ignore[operator]
        ...
        if "image" in instances[0]:
            ...
            batch["images"] = images  # type: ignore[assignment]
...
def train():
    ...
    parser = HfArgumentParser([ModelArguments, DataArguments, TrainingArguments])  # type: ignore[arg-type]
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
    local_rank = training_args.local_rank
    compute_dtype = (
        torch.float16
        if getattr(training_args, "fp16", False)
        else (torch.bfloat16 if getattr(training_args, "bf16", False) else torch.float32)
    )

    bnb_model_from_pretrained_args = {}
    if getattr(training_args, "bits", 16) in [4, 8] and BitsAndBytesConfig is not None:
        bnb_model_from_pretrained_args.update(
            dict(
                device_map={"": getattr(training_args, "device", "cpu")},
                load_in_4bit=getattr(training_args, "bits", 16) == 4,
                load_in_8bit=getattr(training_args, "bits", 16) == 8,
                quantization_config=BitsAndBytesConfig(
                    ...
                    bnb_4bit_use_double_quant=getattr(training_args, "double_quant", True),
                    bnb_4bit_quant_type=getattr(training_args, "quant_type", "nf4"),  # {'fp4', 'nf4'}
                ),
            )
        )
    # ==========================================================================
    if getattr(model_args, "image_tower", None) is not None or getattr(model_args, "video_tower", None) is not None:
        # ==========================================================================
        if "mpt" in getattr(model_args, "model_name_or_path", ""):
            config = transformers.AutoConfig.from_pretrained(
                model_args.model_name_or_path, trust_remote_code=True
            )
            config.attn_config["attn_impl"] = getattr(training_args, "mpt_attn_impl", "triton")
    ...
    if training_args.lora_enable:
        ...
        if training_args.bits == 16:
            if training_args.bf16:
                model.to(torch.bfloat16)  # type: ignore[call-arg]
            if training_args.fp16:
                model.to(torch.float16)  # type: ignore[call-arg]
    ...
    trainer = LLaVATrainer(model=model, args=training_args, **data_module)
