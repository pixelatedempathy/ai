    ...
    def forward(self, batch):
        ...
        if self.modality == "text":
            ...
            outputs = self.pretrain_model(
                input_ids=input_ids, attention_mask=attention_masks, output_hidden_states=True
            ).hidden_states
            ...
            outputs = self.pretrain_model(
                audios, output_hidden_states=True
            ).hidden_states  # tuple * [batch*segment, seqlen, featdim]
            outputs = torch.stack(outputs)[[-1, -2, -3, -4]].sum(
                dim=0
            )  # [batch*segment, seqlen, featdim]
            features = (
                outputs.mean(dim=1).view(bsize, segment, -1).mean(dim=1)
            )  # [batch*segment, featdim] -> [batch, segment, featdim] -> [batch, featdim]
            ...
            if self.model_name in [CLIP_VIT_BASE, CLIP_VIT_LARGE]:
                ...
                outputs = self.pretrain_model.get_image_features(
                    videos
                )  # [bsize * segment, 3, 224, 224] -> [bsize * segment, featdim]
                ...
                num_patches_per_frame = (
                    self.pretrain_model.config.image_size // self.pretrain_model.config.patch_size
                ) ** 2  # 14*14
                outputs = outputs.view(
                    bsize, 16 // self.pretrain_model.config.tubelet_size, num_patches_per_frame, -1
                )  # [bsize, segment=8, patch, featdim]
