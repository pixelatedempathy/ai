...
def normalize_dataset_format(data_root, save_root):
    ...
    ## input path
    train_video, train_label = os.path.join(data_root, "video-labeled"), os.path.join(
        data_root, "label-disdim.csv"
    )
    semi_video, semi_label = os.path.join(data_root, "video-unlabeled"), os.path.join(
        data_root, "semi-label.csv"
    )
    ...
    if not os.path.exists(save_test1):
        os.makedirs(save_test1)
    if not os.path.exists(save_test2):
        os.makedirs(save_test2)

    ## generate label path
    whole_corpus = {}
    for subset, video_root, names, labels, save_root in [
        ("train", train_video, train_names, train_emos, save_train),
