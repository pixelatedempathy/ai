...
def get_image_emotion_batch(
    ...
    if template == "case0":  # 原始的prompt
        ...
        template == "case1"
    ):  # 删除 Please play the role of a facial expression classification expert. 分析其影响
        prompt = [
            {
                "type": "text",
...
# 20 images per time
def get_video_emotion_batch(
    video_paths, candidate_list, sleeptime=0, samplenum=3, model="gpt-4-vision-preview"
):
    prompt = [
        {
            "type": "text",
...
def update_text_reason(
    video_reason, audio_reason, subtitle, sleeptime=0, model="gpt-3.5-turbo-16k-0613"
):
