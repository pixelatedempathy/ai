...
class MERDataset(Dataset):
    ...
    def __init__(self, label_path, audio_root, text_root, video_root, data_type, debug=False):
        assert data_type in ["train", "test1", "test2", "test3"]
        self.name2audio, self.name2labels, self.adim = read_data_multiprocess(
            label_path, audio_root, task="whole", data_type=data_type, debug=debug
        )
        self.name2text, self.name2labels, self.tdim = read_data_multiprocess(
            label_path, text_root, task="whole", data_type=data_type, debug=debug
        )
        self.name2video, self.name2labels, self.vdim = read_data_multiprocess(
            label_path, video_root, task="whole", data_type=data_type, debug=debug
        )
    ...
    def __getitem__(self, index):
        ...
        return (
            torch.FloatTensor(self.name2audio[name]),
    ...
    def get_featDim(self):
        print(
            f"audio dimension: {self.adim}; text dimension: {self.tdim}; video dimension: {self.vdim}"
        )
...
def get_loaders(args, config):
    train_dataset = MERDataset(
        label_path=config.PATH_TO_LABEL[args.train_dataset],
    ...
    for test_set in args.test_sets:
        test_dataset = MERDataset(
            label_path=config.PATH_TO_LABEL[args.test_dataset],
...
class Attention(nn.Module):
    ...
    def forward(self, audio_feat, text_feat, video_feat):
        ...
        multi_hidden2 = torch.stack(
            [audio_hidden, text_hidden, video_hidden], dim=2
        )  # [32, 128, 3]
...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, optimizer=None, train=False):
    ...
    if args.savewhole:
        save_results["embeddings"] = embeddings
    return save_results
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_dataset", type=str, default=None, help="dataset"
    )  # for cross-dataset evaluation
    parser.add_argument(
        "--test_dataset", type=str, default=None, help="dataset"
    )  # for cross-dataset evaluation
    ...
    parser.add_argument(
        ...
        type=str,
        default="test1,test2",
        help="process on which test sets, [test1, test2, test3]",
    )
    parser.add_argument(
        "--save_root", type=str, default="./saved", help="save prediction results and models"
    )
    parser.add_argument(
        "--savewhole", action="store_true", default=False, help="whether save latent embeddings"
    )

    ## Params for model
    parser.add_argument(
        "--layers", type=str, default="256,128", help="hidden size in model training"
    )
    parser.add_argument(
        "--n_classes", type=int, default=-1, help="number of classes [defined by args.label_path]"
    )
    parser.add_argument(
        "--num_folder",
        type=int,
    ...
    parser.add_argument(
        "--model_type", type=str, default="mlp", help="model type for training [mlp or attention]"
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=100, help="make split manner is same with same seed"
    )
    ...
    for ii in range(args.num_folder):
        ...
        for epoch in range(args.epochs):
            ...
            eval_results = train_or_eval_model(
                args, model, reg_loss, cls_loss, eval_loader, optimizer=None, train=False
            )
            eval_metric = overall_metric(
                eval_results["emo_fscore"], eval_results["val_mse"]
            )  # bigger -> better
            ...
            print(
                "epoch:%d; train_fscore:%.4f; eval_metric:%.4f"
                % (epoch + 1, train_results["emo_fscore"], eval_metric)
            )
            ...
            for jj, test_loader in enumerate(test_loaders):
                ...
                if args.savewhole:
                    store_values[f"{test_set}_embeddings"] = test_results["embeddings"]
    ...
    if not os.path.exists(save_predroot):
        os.makedirs(save_predroot)
    ...
    for setname in args.test_sets:
        ...
        if args.savewhole:
            name2feats = gain_name2feat(folder_save, setname)
        write_to_csv_pred(name2preds, pred_path)

        res_name = "nores"
        if os.path.exists(label_path):
            if setname in ["test1", "test2"]:
                emo_fscore, val_mse, final_metric = report_results_on_test1_test2(
                    label_path, pred_path
                )
            if setname in ["test3"]:
                emo_fscore, val_mse, final_metric = report_results_on_test3(label_path, pred_path)
        ...
        if args.savewhole:
            ...
            np.savez_compressed(save_path, name2preds=name2preds, args=np.array(args, dtype=object))
