...
CLIP_CHINESE = (
    "taiyi-clip-roberta-chinese"  # https://huggingface.co/IDEA-CCNL/Taiyi-CLIP-Roberta-102M-Chinese
)
WENZHONG_GPT2_CHINESE = (
    "wenzhong2-gpt2-chinese"  # https://huggingface.co/IDEA-CCNL/Wenzhong2.0-GPT2-3.5B-chinese
)
...
def extract_bert_embedding_chinese(
    model_name, trans_dir, save_dir, feature_level, layer_ids=None, gpu=6, overwrite=False
):
    ...
    if feature_level == "FRAME":
        save_dir = os.path.join(save_dir, dir_name + "-FRA")
    if feature_level == "UTTERANCE":
        save_dir = os.path.join(save_dir, dir_name + "-UTT")
    if not os.path.exists(save_dir):
        ...
        raise Exception(
            f'==> Error: csv out dir "{dir_name}" already exists, set overwrite=TRUE if needed!'
        )
...
## 英文涉及到句子切分等功能
def extract_bert_embedding_english(
    model_name,
    trans_dir,
    save_dir,
    feature_level,
    ...
    )  # do not use fast version of Tokenizer (because of failure of RoBERTa )
    ...
    for idx, row in df.iterrows():
        ...
        for word in words:
            if word in [".", "!", "?"]:
                ...
                if (
                    "uncased" in model_name or "albert" in model_name or "electra" in model_name
                ):  # Note: to lower case if needed, different from previous version, 2021/05/04
        ...
        for i in range(n_batches):
            ...
            inputs = tokenizer(
                batch_sentences, padding=True, is_split_into_words=True, return_tensors="pt"
            )
            ...
            with torch.no_grad():
                ...
                for i in range(real_batch_size):  # for each sample
                    if "xlnet" in model_name:
                        input_id = input_ids[
                            i, -lens[i] : -2
                        ]  # (T,) Note, for left pading and skipping ('<sep>', '<cls>')
                        output = outputs[i, -lens[i] : -2]  # (T, D)
                    else:  # lens[0] = 21
                        input_id = input_ids[
                            i, 1 : (lens[i] - 1)
                        ]  # (T,) Note, 1:(lens[i] - 1) for skipping [CLS] and [SEP]
                        output = outputs[i, 1 : (lens[i] - 1)]  # (T, D)
                    sentence = batch_sentences[i]
                    n_tokens, n_words = output.shape[0], len(
                        sentence
                    )  ##  I'm => [I, ', m] word can be split into sub-words
                    ...
                    if n_tokens == n_words:  # sub-word is word
                        ...
                        for j, token_id in enumerate(input_id):
                            ...
                            token = token.replace(
                                "▁", ""
                            )  #  for albert-like (also, xlnet) model (ex, hello: '▁hello')
                            token = token.replace(
                                "Ġ", ""
                            )  #  for roberta-like (also, gpt) model (ex, was: 'Ġwas')
                            if (
                                token == current_word or token == "[UNK]"
                                ...
                                if word == current_word:  # ended token
                                    if combine_type == "sum":
                                        word_embedding = np.sum(
                                            np.row_stack(word_embedding), axis=0
                                        )  # sum sub-word emebddings
                                    elif combine_type == "mean":
                                        word_embedding = np.mean(
                                            np.row_stack(word_embedding), axis=0
                                        )  # average sub-word emebddings
                                    elif combine_type == "last":
                                        word_embedding = word_embedding[
                                            -1
                                        ]  # take the last sub-word emebdding
                                    else:
                                        raise Exception(
                                            "Error: not supported type to combine subword embedding."
                                        )
                                    sentence_embedding.append(word_embedding)
                                    sentence_words.append(word)
                                    word, word_embedding = "", []
                                    pointer += 1
                        assert len(sentence) == len(sentence_embedding), print(
                            f"==>len(sentence): {len(sentence)}, len(embedding): {len(sentence_embedding)}\ntokens:{sentence_tokens}\nwords:{sentence_words}\nsentence:{sentence}"
                        )
                        embeddings.extend(sentence_embedding)
...
def extract_word_embedding_english(
    model_name, trans_dir, save_dir, feature_level, dir_name=None, overwrite=False
):
    ...
    if feature_level == "FRAME":
        save_dir = os.path.join(save_dir, dir_name + "_FRA")
    if feature_level == "UTTERANCE":
        save_dir = os.path.join(save_dir, dir_name + "_UTT")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    elif overwrite or len(os.listdir(save_dir)) == 0:
        print(f'==> Warning: overwrite csv out dir "{dir_name}"!')
...
def main(model_name, feature_level, trans_dir, save_dir, layer_ids, overwrite, gpu, batch_size):
    ...
    if model_name.find("chinese") != -1:
        extract_bert_embedding_chinese(
        ...
        extract_word_embedding_english(
            model_name=model_name,
            trans_dir=trans_dir,
            save_dir=save_dir,
            feature_level=feature_level,
        ...
        extract_bert_embedding_english(
            model_name=model_name,
            trans_dir=trans_dir,
            save_dir=save_dir,
            layer_ids=layer_ids,
...
if __name__ == "__main__":
    ...
    layer_ids = [
        ...
        -2,
        -1,
    ]  # hidden states of selected layers will be used to obtain the embedding, only for transformers
