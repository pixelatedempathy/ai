...
class CROSSDIS:
    ...
    def get_loaders(self):
        ...
        if self.train_dataset in ["IEMOCAPFour", "IEMOCAPSix"]:
            names, labels = self.read_names_labels(
                self.train_dataset, data_type="whole", debug=self.debug
            )
            ...
            names, labels = self.read_names_labels(
                self.train_dataset, data_type="train", debug=self.debug
            )
            ...
            train_loaders.append(train_loader)
            names, labels = self.read_names_labels(
                self.train_dataset, data_type="val", debug=self.debug
            )
            dataset = get_datasets(self.args, names, labels)
            eval_loader = DataLoader(
                dataset,
    ...
    def read_names_labels(self, dataset, data_type, debug=False):
        ...
        if data_type == "whole":
            corpus = np.load(label_path, allow_pickle=True)["whole_corpus"].tolist()  # for IEMOCAP
        if data_type == "test1":
            corpus = np.load(label_path, allow_pickle=True)["test1_corpus"].tolist()  # for MER2023
