...
def normalize_dataset_format(data_root, save_root):
    ...
    ## generate label path
    whole_corpus = {}
    for subset, video_root, names, labels in [
        ("train", train_video, train_names, train_emos),
    ...
    ## 需要对视频进行加噪处理
    np.savez_compressed(
        save_label, train_corpus=whole_corpus["train"], test1_corpus=whole_corpus["test1"]
    )
