...
class Encoder(nn.Module):  # Encoder
    ...
    def forward(self, x, lengths):
        ...
        if self.bidirectional:
            h = self.dropout(
                torch.add(enc_output[:, :, : self.hidden_dim], enc_output[:, :, self.hidden_dim :])
            )
        else:
    ...
    def forward(self, dec_input, s, join):
        ...
        if self.bidirectional:
            dec_output = torch.add(
                dec_output[:, :, : self.hidden_dim], dec_output[:, :, self.hidden_dim :]
            )
...
class MCTN(nn.Module):
    def __init__(self, args):
        ...
        self.decoder1 = Decoder(
            self.output_dim, hidden_dim, dropout, depth[1], self.attn1, bidirectional=True
        )
        ...
        self.decoder2 = Decoder(
            self.output_dim, hidden_dim, dropout, depth[1], self.attn2, bidirectional=True
        )
