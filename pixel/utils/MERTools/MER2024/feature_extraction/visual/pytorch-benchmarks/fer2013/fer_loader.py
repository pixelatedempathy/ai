...
class Fer2013Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, mode="val", transform=None, include_train=False):
...
class Fer2013PlusDataset(Fer2013Dataset):
    ...
    def update_labels(self):
        ...
        val_keep_labels = np.logical_and(sets == 2, np.logical_not(to_drop)).flatten()
        test_keep_labels = np.logical_and(sets == 3, np.logical_not(to_drop)).flatten()
        ...
        self.data["images"]["val"] = self.data["images"]["val"][val_keep_ims, :, :, :]
        self.data["images"]["test"] = self.data["images"]["test"][test_keep_ims, :, :, :]
