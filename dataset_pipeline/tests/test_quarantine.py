...
from ai.dataset_pipeline.quarantine import (
    QuarantineRecord,
    QuarantineStatus,
    QuarantineStore,
)
...
def test_quarantine_store_connection(mocker):
    mocker.patch("ai.dataset_pipeline.quarantine.MongoClient")
    mocker.patch("ai.dataset_pipeline.quarantine.MongoClient.admin.command")
    store = QuarantineStore(mongo_uri="mock://localhost")
    assert store.collection is not None  # Connection succeeds


@patch("ai.dataset_pipeline.quarantine.MongoClient")
...
def test_quarantine_store_reprocess_success(mock_client, mocker):
    ...
    mocker.patch("ai.dataset_pipeline.quarantine.IngestRecord")
    mocker.patch("ai.dataset_pipeline.quarantine.datetime.utcnow")
    mocker.patch("ai.dataset_pipeline.quarantine.validate_record", return_value=MagicMock())
...
def test_quarantine_store_reprocess_fail(mock_client, mocker):
    ...
    mocker.patch("ai.dataset_pipeline.quarantine.IngestRecord")
    mocker.patch("ai.dataset_pipeline.quarantine.validate_record", side_effect=ValueError("Invalid"))
