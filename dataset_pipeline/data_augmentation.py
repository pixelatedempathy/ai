...
Implements paraphrasing, contextual augmentation, and noise injection with appropriate guardrails.
"""
...
class SafetyGuardrails:
    ...
    def __init__(self):
        ...
        self.sensitive_keywords = {
            'crisis': [r'(?i)\bsuicid', r'(?i)\bkill.*myself', r'(?i)\bend.*life', 
                      r'(?i)\bcan\'t.*go.*on', r'(?i)\bharm.*myself'],
            'safety': [r'(?i)\bhelp.*line', r'(?i)\bcrisis.*text', r'(?i)\bemergency.*number',
                      r'(?i)\bcall.*911', r'(?i)\bemergency.*services'],
            'therapeutic': [r'(?i)\btherapist', r'(?i)\bcounselor', r'(?i)\btherapist',
                           r'(?i)\bmental.*health', r'(?i)\btherapy']
        }
        
        # Context-preserving phrases that should be handled carefully
        self.context_phrases = [
            r'(?i)\bi.*understand', r'(?i)\byou.*mentioned', r'(?i)\bit.*sounds.*like',
            r'(?i)\bhow.*can.*i.*help', r'(?i)\bwhat.*brings.*you.*here'
        ]

    def validate_augmentation(self, original_text: str, augmented_text: str) -> Tuple[bool, List[str]]:
        ...
        for category, patterns in self.sensitive_keywords.items():
            for pattern in patterns:
                if re.search(pattern, original_text):
                    if not re.search(pattern, augmented_text) and category in ['crisis', 'safety']:
                        issues.append(f"CRITICAL: {category} keyword lost in augmentation")
        
        # Check for major therapeutic context preservation
        context_preserved = True
        for phrase_pattern in self.context_phrases:
            if re.search(phrase_pattern, original_text) and not re.search(phrase_pattern, augmented_text):
                # This may not be an issue depending on augmentation type
                if "therapist" in original_text.lower() and "therapist" not in augmented_text.lower():
        ...
        if len(original_tokens) > 0:
            ...
            if similarity < 0.3:  # Less than 30% overlap
                issues.append(f"CONTENT_INTEGRITY: Low semantic overlap ({similarity:.2f})")
...
class DataAugmenter:
    ...
    def _initialize_paraphrase_templates(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            # Probing questions
            'probing': [
                ("Can you tell me more about", "What else comes up when you think about"),
    ...
    def _initialize_contextual_variations(self) -> Dict[str, List[str]]:
        ...
        return {
            'setting': [
                ("in our session", "today"), ("during our time together", "when we talk"), 
                ("in therapy", "in our conversations")
            ],
            'time_ref': [
                ("recently", "lately"), ("in the past", "before"), ("currently", "right now"),
                ("sometimes", "at times"), ("often", "frequently")
            ],
            'emotional_intensity': [
                ("very", "really"), ("quite", "pretty"), ("extremely", "incredibly"),
                ("somewhat", "a bit"), ("fairly", "relatively")
            ]
        }
    ...
    def _initialize_demographic_variations(self) -> Dict[str, List[Dict[str, str]]]:
        ...
        return {
            'age': [
                {"old": ["young", "teen", "adolescent"], "new": ["adult", "mature", "experienced"]},
                {"old": ["elderly", "senior", "older"], "new": ["middle-aged", "adult", "contemporary"]}
            ],
            'gender': [
    ...
    def paraphrase_text(self, text: str) -> str:
        ...
        if not self.config.paraphrase_enabled or random.random() > self.config.paraphrase_probability:
            return text
        ...
        for category, templates in self.paraphrase_templates.items():
            for original, replacement in templates:
                ...
                if re.search(re.escape(original), augmented_text, re.IGNORECASE):
                    ...
                    if original.lower().startswith(augmented_text[:len(original)].lower()):
                        replacement = replacement.capitalize()
                    augmented_text = re.sub(re.escape(original), replacement, augmented_text, flags=re.IGNORECASE, count=1)

        return augmented_text

    def contextual_augmentation(self, text: str) -> str:
        """Apply contextual variations to enrich the conversation"""
        if not self.config.contextual_augmentation_enabled or random.random() > self.config.contextual_probability:
            return text
        ...
        for category, variations in self.contextual_variations.items():
            for old_phrase, new_phrase in variations:
                if random.random() < 0.5:  # Randomly choose to apply or not
                    augmented_text = re.sub(re.escape(old_phrase), new_phrase, augmented_text, flags=re.IGNORECASE)
        
        return augmented_text

    def demographic_variation(self, text: str) -> str:
        """Apply demographic variations to increase diversity"""
        if not self.config.demographic_variation_enabled or random.random() > self.config.demographic_variation_probability:
            return text
            ...
            for variation in variations_list:
                ...
                if len(old_words) == len(new_words):
                    for old_word, new_word in zip(old_words, new_words):
                        ...
                        if re.search(r'\b' + re.escape(old_word) + r'\b', augmented_text, re.IGNORECASE):
                            augmented_text = re.sub(r'\b' + re.escape(old_word) + r'\b', new_word, 
                                                  augmented_text, flags=re.IGNORECASE)
        
        return augmented_text

    def inject_noise(self, text: str) -> str:
        """Inject small amounts of noise for robustness"""
        if not self.config.noise_injection_enabled or random.random() > self.config.noise_probability:
            return text
    ...
    def augment_conversation(self, conversation: Conversation) -> Conversation:
        ...
        if not self.config.safety_guardrails_enabled:
            logger.warning("Safety guardrails are disabled. This is not recommended for production use.")
        ...
        for message in conversation.messages:
            ...
            if self.config.safety_guardrails_enabled:
                is_valid, issues = self.guardrails.validate_augmentation(original_content, augmented_content)
                if not is_valid:
                    ...
                    if critical_issues:
                        logger.warning("Reverting to original content due to critical safety issues")
        ...
        augmented_conversation.metadata["augmentation_types"] = self._get_applied_augmentation_types()
        augmented_conversation.metadata["augmentation_timestamp"] = augmented_conversation.updated_at
    ...
    def augment_label_bundle(self, original_bundle: LabelBundle, augmented_conversation: Conversation) -> LabelBundle:
        ...
        if augmented_bundle.crisis_label:
            augmented_bundle.crisis_label.metadata.additional_context["source_augmented"] = True
        if augmented_bundle.therapy_modality_label:
            augmented_bundle.therapy_modality_label.metadata.additional_context["source_augmented"] = True
        if augmented_bundle.mental_health_condition_label:
            augmented_bundle.mental_health_condition_label.metadata.additional_context["source_augmented"] = True
        if augmented_bundle.demographic_label:
            augmented_bundle.demographic_label.metadata.additional_context["source_augmented"] = True
            
        return augmented_bundle

    def batch_augment(self, conversations: List[Conversation], 
                     label_bundles: Optional[List[LabelBundle]] = None) -> Tuple[List[Conversation], Optional[List[LabelBundle]]]:
        ...
        for i, conversation in enumerate(conversations):
            ...
            if label_bundles and i < len(label_bundles):
                augmented_bundle = self.augment_label_bundle(label_bundles[i], augmented_conv)
...
class AugmentationQualityController:
    ...
    def calculate_augmentation_quality(self, original_text: str, augmented_text: str) -> Dict[str, float]:
    ...
    def validate_augmentation_batch(self, original_convs: List[Conversation], 
                                  augmented_convs: List[Conversation]) -> Dict[str, Any]:
        """Validate quality of a batch of augmented conversations"""
        if len(original_convs) != len(augmented_convs):
            raise ValueError("Original and augmented conversation lists must have the same length")
        ...
        for orig_conv, aug_conv in zip(original_convs, augmented_convs):
            ...
            for orig_msg, aug_msg in zip(orig_conv.messages, aug_conv.messages):
                metrics = self.calculate_augmentation_quality(orig_msg.content, aug_msg.content)
        ...
        if len(original_convs) > 0:
            all_metrics['semantic_preservation_avg'] = total_semantic / len(original_convs)
            all_metrics['length_preservation_avg'] = total_length / len(original_convs)
            all_metrics['diversity_introduced_avg'] = total_diversity / len(original_convs)
        
        return all_metrics


def create_default_augmenter(config: Optional[AugmentationConfig] = None) -> DataAugmenter:
...
def test_data_augmentation():
    ...
    conversation.add_message("therapist", "I understand how you're feeling. It sounds like you've been going through a really difficult time lately.")
    conversation.add_message("client", "Yes, I just can't seem to get out of this rut. I feel hopeless most days.")
    conversation.add_message("therapist", "That must be incredibly hard. Can you tell me more about what's been happening?")
    ...
    quality_metrics = controller.validate_augmentation_batch([conversation], [augmented_conversation])
    print(f"\nQuality metrics: {quality_metrics}")
    
    # Test batch augmentation
    conversations = [conversation] * 3
    augmented_batch, _ = augmenter.batch_augment(conversations)
    print(f"\nBatch augmentation: {len(conversations)} conversations -> {len(augmented_batch)} augmented conversations")
