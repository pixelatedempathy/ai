...
class LocalDryRunTester:
    ...
    def test_package_structure(self) -> bool:
        ...
        if missing_files:
            ...
            self.test_results["package_structure"] = {"passed": False, "missing_files": missing_files}
            return False
    ...
    def test_configuration_files(self) -> bool:
        ...
        for config_file in config_files:
            try:
                with open(self.package_root / config_file, 'r') as f:
                    config = json.load(f)
    ...
    def test_data_files(self) -> bool:
        ...
        if ultimate_file.exists():
            ultimate_stats = self._test_jsonl_file(ultimate_file, "ultimate dataset", sample_size=100)
    ...
    def _test_jsonl_file(self, file_path: Path, description: str, sample_size: int = None) -> Dict[str, Any]:
                ...
                for line_num, line in enumerate(f, 1):
                    if line.strip():
                        ...
                        try:
                            ...
                            # Collect samples
                            if sample_size and len(stats["sample_conversations"]) < sample_size:
                                stats["sample_conversations"].append(conv_data)
                            elif not sample_size and len(stats["sample_conversations"]) < 5:
                                stats["sample_conversations"].append(conv_data)
        ...
        logger.info(f"    📊 {description}: {stats['valid_json_lines']}/{stats['total_lines']} valid ({validity_ratio:.1%})")
    ...
    def _validate_kan28_components(self, component_file: Path) -> Dict[str, Any]:
                        ...
                        try:
                            ...
                            if "integration_metadata" in conv:
                                ...
                                for component in components:
                                    validation["components_found"][component] = validation["components_found"].get(component, 0) + 1
                            ...
                            if "expert_voices" in conv:
                                ...
                                if isinstance(expert_voices, dict):
                                    for expert in expected_experts:
                                        expert_key = expert.split()[0].lower()  # Use first name
                                        if any(expert_key in key.lower() for key in expert_voices.keys()):
                                            validation["expert_voices_found"][expert] = validation["expert_voices_found"].get(expert, 0) + 1
                            
                            # Collect sample integration
                            if len(validation["sample_integrations"]) < 3:
                                sample = {
                                ...
                                validation["sample_integrations"].append(sample)
        ...
        missing_components = [comp for comp in expected_components if comp not in validation["components_found"]]
        missing_experts = [expert for expert in expected_experts if expert not in validation["expert_voices_found"]]
        
        if missing_components:
            logger.warning(f"    ⚠️ Missing components: {missing_components}")
        if missing_experts:
            logger.warning(f"    ⚠️ Missing expert voices: {missing_experts}")
        
        logger.info(f"    ✅ Components found: {list(validation['components_found'].keys())}")
        logger.info(f"    ✅ Expert voices found: {list(validation['expert_voices_found'].keys())}")
    ...
    def test_training_config_compatibility(self) -> bool:
        ...
        try:
            with open(self.package_root / "config/enhanced_training_config.json", 'r') as f:
                config = json.load(f)
            ...
            compatibility_checks = {
                ...
                "reasonable_batch_size": config.get("training_parameters", {}).get("per_device_train_batch_size", 0) <= 8,
                "reasonable_learning_rate": 1e-5 <= float(config.get("training_parameters", {}).get("learning_rate", 0)) <= 1e-3
            }
    ...
    def generate_test_report(self) -> str:
        ...
        test_report = {
            ...
            "next_steps": [
                "Upload package to Lightning.ai Studio" if overall_success else "Fix failing tests first",
...
def main():
    ...
    if passed_tests == total_tests:
        print("🎉 ALL TESTS PASSED - READY FOR LIGHTNING.AI!")
        print("📋 Next steps:")
        print("  1. Upload package to Lightning.ai Studio")
