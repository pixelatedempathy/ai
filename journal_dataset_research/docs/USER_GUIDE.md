...
```bash
python -m ai.journal_dataset_research.cli.cli --help
```
...
```bash
python -m ai.journal_dataset_research.cli.cli integrate \
    --session-id "my_session" \
...
```python
from ai.journal_dataset_research.acquisition.acquisition_manager import AcquisitionManager
