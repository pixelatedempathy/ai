...
def export_to_gguf(model_path, output_dir):
    ...
    print(f"🚀 Exporting {model_path} to GGUF formats...")
    ...
    print(f"📦 Converting to base GGUF: {base_gguf}")
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"❌ Base conversion failed: {result.stderr}")
            return False
    ...
    for quant_name, quant_type, description in quant_configs:
        output_file = output_path / f"Wayfarer2-Pixelated-{quant_name}.gguf"
        print(f"📦 Creating {description}: {output_file}")
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"✅ {quant_name} quantization complete")
            else:
                print(f"❌ {quant_name} quantization failed: {result.stderr}")
                
        except Exception as e:
            print(f"❌ {quant_name} quantization error: {e}")
    
    print(f"🎯 GGUF export complete. Files in: {output_path}")
    return True
...
def main():
    ...
    if success:
        print("✅ All GGUF exports completed successfully")
    else:
        print("❌ Some GGUF exports failed")
