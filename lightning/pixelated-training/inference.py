...
class WayfarerInference:
    def __init__(self, model_path="./wayfarer-finetuned"):
        print(f"🚀 Loading Wayfarer model from {model_path}")
    ...
    def chat_loop(self):
        ...
        print("💬 Wayfarer Chat (type 'quit' to exit)")
        print("=" * 40)
        ...
        while True:
            ...
            print(f"Wayfarer: {result['response']}")
            print(f"({result['generation_time']:.2f}s, {result['tokens_generated']} tokens)\n")
...
def main():
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "--chat":
        ...
        result = wayfarer.generate_response(test_message)
