...
class S3DatasetDownloader:
    """Downloads datasets directly to S3 (OVH or AWS-compatible)"""
    ...
    def __init__(self, s3_bucket: str, s3_prefix: str = "datasets/", catalog_path: Optional[Path] = None):
        ...
        # Prefer OVH object storage env vars, fall back to generic/AWS-style ones so this
        # works both against OVH Cloud and vanilla AWS S3.
        ...
        if BOTO3_AVAILABLE:
            ...
            self.s3_client = None
            logger.error("boto3 not available. Cannot upload to S3 / OVH object storage.")
...
def main():
    ...
    # Check for OVH/AWS-style S3 credentials
    have_ovh_creds = bool(os.getenv("OVH_S3_ACCESS_KEY") and os.getenv("OVH_S3_SECRET_KEY"))
    have_aws_creds = bool(os.getenv("AWS_ACCESS_KEY_ID") and os.getenv("AWS_SECRET_ACCESS_KEY"))
    have_dataset_creds = bool(
        os.getenv("DATASET_S3_ACCESS_KEY_ID") and os.getenv("DATASET_S3_SECRET_ACCESS_KEY")
    )

    if not (have_ovh_creds or have_aws_creds or have_dataset_creds):
        logger.error(
            "❌ S3 credentials not found. Set OVH_S3_ACCESS_KEY/OVH_S3_SECRET_KEY "
            "for OVH object storage, or AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY, "
            "or DATASET_S3_ACCESS_KEY_ID/DATASET_S3_SECRET_ACCESS_KEY."
        )
        return 1
