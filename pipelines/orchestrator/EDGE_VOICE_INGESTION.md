...
1. **Generate / refresh edge nightmare data**
   ...
   ```
2. **Verify output** – should appear at `ai/pipelines/edge_case/output/edge_cases_training_format.jsonl`.
3. **Loader mapping**
...
5. **Scenario prompt corpus**
   - Mirror Google Drive sheets to `ai/pipelines/orchestrator/prompt_corpus/{category}.md`.
   - Use prompts to seed `SyntheticDataDistillationPipeline(strategy="edge_case_expansion")`.
6. **Safety DPO pairs**
   ```python
   from ai.pipelines.orchestrator.quality.safety_alignment_validator import create_safety_dpo_dataset
   unsafe_pairs = create_safety_dpo_dataset(edge_conversations, validator)
   ```
   Save to `ai/pipelines/orchestrator/quality/output/safety_dpo_pairs.jsonl` (manifest entry `safety_dpo_pairs`).
...
4. **Dual persona stitching**
   - `ai/pipelines/dual_persona` output should include `persona_id` metadata before entering Stage 4 bucket.
