...
class PersonalityExtractor:
    ...
    def extract_personality(self, conversation: dict[str, Any]) -> PersonalityProfile:
        ...
        confidence = min(
            0.8, len(content.split()) / 100
        )  # Simple confidence based on content length
    ...
    def _analyze_openness(self, content: str) -> float:
        ...
        score = sum(
            1 for indicator in openness_indicators if indicator in content.lower()
        )
    ...
    def _analyze_conscientiousness(self, content: str) -> float:
        ...
        score = sum(
            1
            for indicator in conscientiousness_indicators
    ...
    def _analyze_extraversion(self, content: str) -> float:
        ...
        score = sum(
            1 for indicator in extraversion_indicators if indicator in content.lower()
        )
        return min(1.0, score / 5)
    ...
    def _analyze_agreeableness(self, content: str) -> float:
        ...
        score = sum(
            1 for indicator in agreeableness_indicators if indicator in content.lower()
        )
        return min(1.0, score / 5)
    ...
    def _analyze_neuroticism(self, content: str) -> float:
        ...
        score = sum(
            1 for indicator in neuroticism_indicators if indicator in content.lower()
        )
        return min(1.0, score / 5)
...
class PersonalityAdapter:
    ...
    def _load_communication_patterns(self) -> dict[str, dict[str, list[str]]]:
        ...
        return {
            ...
            "dark_humor": {
                "openings": [
                    ("Well, isn't this a delightful mess we've found ourselves in."),
                    ("Look, life's a tragedy, we might as well embrace the absurdity."),
                    (
                        "So, the universe decided to use you as a punching "
                        "bag today. Classic."
                    ),
                ],
                "transitions": [
                    "And just when you thought it couldn't get more ridiculous...",
                    "Here's the fun part of this grim reality:",
                    "Let's peel back another layer of this beautiful disaster:",
                ],
                "closings": [
                    "Keep your head up. Or don't. I'm not the boss of you.",
                    "We'll figure this out. Probably. Unless we don't, which "
                    "would be awkward.",
    ...
    def _load_therapeutic_mappings(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "provocative": {
                "focus": "emotional_resilience",
                "techniques": [
                    "humor",
                    "paradoxical_intention",
                    "advocating_status_quo",
                ],
                "language_style": "provocative_humorous",
                "session_structure": "unconventional",
            },
        }

    def analyze_personality_for_adaptation(
        self, conversation: dict[str, Any]
    ) -> PersonalityAdaptation:
        """Analyze personality and generate adaptation recommendations."""
        try:
            # Extract personality profile
            personality_profile = self.personality_extractor.extract_personality(
                conversation
            )
            ...
            logger.info(
                "Generated personality adaptation with "
                f"{adaptation.adaptation_confidence:.2f} confidence"
            )
            return adaptation
    ...
    def _get_safe_therapeutic_approach(self, value: str) -> TherapeuticApproach:
        """Safely get therapeutic approach from string."""
        try:
            return TherapeuticApproach(value)
        except ValueError:
            logger.warning(
                f"Invalid therapeutic approach: '{value}'. Falling back to HUMANISTIC."
            )
    ...
    def _generate_adaptation(
        ...
        adaptation = PersonalityAdaptation(
            ...
            communication_style=CommunicationStyle(
                rules.get("communication_style", "supportive")
            ),
            therapeutic_approach=self._get_safe_therapeutic_approach(
                rules.get("therapeutic_approach", "humanistic")
            ),
    ...
    def _generate_recommendations(self, adaptation: PersonalityAdaptation) -> list[str]:
        ...
        if adaptation.communication_style == CommunicationStyle.DIRECT:
            recommendations.append(
                "Use clear, concise language and get straight to the point"
            )
        ...
        elif adaptation.communication_style == CommunicationStyle.DARK_HUMOR:
            recommendations.append("Use irony and dark humor to deflate tension")
            recommendations.append("Be brutally honest but charmingly supportive")
            recommendations.append("Frame challenges as absurd or existential jokes")
        ...
        if adaptation.therapeutic_approach == TherapeuticApproach.CBT:
            ...
            recommendations.append(
                "Emphasize emotion regulation and distress tolerance skills"
            )
            recommendations.append(
                "Validate intense emotions while teaching coping strategies"
            )
            ...
            recommendations.append(
                "Use reflective listening and avoid giving direct advice"
            )
    ...
    def adapt_response(
        self, original_response: str, adaptation: PersonalityAdaptation
    ) -> AdaptedResponse:
        ...
        try:
            ...
            return AdaptedResponse(
                ...
                adaptation_notes=(
                    f"Adapted for {adaptation.communication_style.value} "
                    "communication style"
                ),
            )
    ...
    def _apply_adaptation_rules(
        self, response: str, adaptation: PersonalityAdaptation
    ) -> str:
        ...
        if adaptation.communication_style == CommunicationStyle.DIRECT:
            ...
            adapted_response = self._make_dark_humor(adapted_response)
    ...
    def _make_more_supportive(self, response: str) -> str:
        ...
        if not any(
            ...
            for phrase in ["understand", "normal", "strength"]
        ):
    ...
    def _make_more_analytical(self, response: str) -> str:
        ...
        if not response.startswith(
            ("Let's examine", "If we analyze", "Looking at this systematically")
        ):
    ...
    def _make_dark_humor(self, response: str) -> str:
        """Make response employ dark humor and charming cockiness."""
        # Prefixes that set the tone
        dark_prefixes = [
            "Look, I'm not going to sugarcoat this disaster. ",
            "Well, that's one way to ruin a perfectly good Tuesday. ",
            "If it makes you feel better, the universe eventually mocks us all. ",
        ]

        # Suffixes that add a twist
        dark_suffixes = [
            " But hey, at least you have excellent taste in crises.",
            " On the bright side, it can't get much worse. Theoretically.",
            " We'll fix it. I'm disgustingly good at my job.",
        ]

        # Inject tone if not present
        if not any(phrase[:10] in response for phrase in dark_prefixes):
            import random

            response = random.choice(dark_prefixes) + response

        # Add a charming/cocky closer occasionally
        import random

        if random.random() > 0.5 and not any(
            phrase[:10] in response for phrase in dark_suffixes
        ):
            response = response.rstrip(".") + "." + random.choice(dark_suffixes)

        # Replace standard platitudes with darker equivalents
        replacements = {
            "I understand how you feel": "I see the abyss you're staring into",
            "It will get better": "Survival is optional, but highly recommended",
            "You are strong": "You're too stubborn to quit, which I respect",
            "Take a deep breath": (
                "Don't forget to breathe. Oxygen is still free, surprisingly"
            ),
        }
    ...
    def _add_more_detail(self, response: str) -> str:
        ...
        if len(response.split()) < 20:
            response += (
                " Let me explain this in more detail to help you understand the "
                "full picture."
            )
    ...
    def get_adaptation_summary(
        self, adaptation: PersonalityAdaptation
    ) -> dict[str, Any]:
...
def main():
        ...
        "turns": [
            {
                "speaker": "user",
                "content": (
                    "I've been feeling really overwhelmed lately. I like to have "
                    "everything planned out and organized, but work has been so "
                    "chaotic. I prefer to work alone and think things through "
            ...
            },
            {
                "speaker": "assistant",
                "content": (
                    "I understand you're feeling overwhelmed. It sounds like the "
                    "lack of structure at work is really challenging for you."
                ),
            },
        ],
    }
    ...
    original_response = (
        "Maybe you could try to organize your tasks better. It might help to "
        "make a schedule."
    )
