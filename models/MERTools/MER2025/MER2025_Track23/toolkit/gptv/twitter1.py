...
from toolkit.utils.functions import func_plot_confusion_matrix
...
def evaluate_performance_using_gpt4v(
    image_root, save_root, save_order, bsize=20, batch_flag="flag1", sleeptime=0, xishus=[2, 2, 2]
):
    # params assert
    if len(xishus) == 1:
        assert batch_flag in ["flag1", "flag2"]
    if len(xishus) == 2:
        assert batch_flag in ["flag1", "flag2", "flag3"]
    if len(xishus) == 3:
        assert batch_flag in ["flag1", "flag2", "flag3", "flag4"]
    multiple = 1
    for item in xishus:
        multiple *= item
...
def get_results_and_update_label(gpt4v_root, label_path, store_path):
    ...
    whole_names, whole_gpt4vs = func_remove_dulpicate(
        whole_names, whole_gpt4vs
    )  # remove multiple predicted
    whole_names, whole_gpt4vs = func_remove_falsepreds(
        whole_names, whole_gpt4vs
    )  # remove remove false preds
...
def func_find_correct_samples(gpt4v_csv, image_root, store_root):
    ...
    if not os.path.exists(true_root):
        os.makedirs(true_root)
    if not os.path.exists(false_root):
