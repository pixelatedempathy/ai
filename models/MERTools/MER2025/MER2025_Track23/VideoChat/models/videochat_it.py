...
class VideoChat_it(Blip2Base):
    ...
    def __init__(self, config):
        ...
        gmhra_drop_path_rate = config.get("gmhra_drop_path_rate", 0.0)
    ...
    def forward(self, image, text_input):
        ...
        for idx, prompt in enumerate(text_input):
            ...
            ), f"The final length is not equal to the original prompt: {prompt}"
        ...
        targets = (
            torch.ones([batch_size, txt_len], dtype=torch.long).to(img_embeds.device).fill_(-100)
        )
        # set bos_token
        ...
        for idx in range(batch_size):
            ...
            targets[idx, (p_before_len + img_len + 1) : (input_len + 1)] = target_list[idx][
