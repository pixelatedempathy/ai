...
def extract_openset_batchcalling(
    ...
    if store_root is not None:
        ...
        np.savez_compressed(store_npz, filenames=whole_names, fileitems=whole_responses)
    else:
...
if __name__ == "__main__":
    ...
    for result_path in glob.glob("output/results-mer2025ov/*/*.npz"):
        ...
        level1_fscore, _, _ = wheel_metric_calculation(
            name2gt=name2gt, name2pred=name2pred, inter_print=False, level="level1"
        )
        level2_fscore, _, _ = wheel_metric_calculation(
            name2gt=name2gt, name2pred=name2pred, inter_print=False, level="level2"
        )
