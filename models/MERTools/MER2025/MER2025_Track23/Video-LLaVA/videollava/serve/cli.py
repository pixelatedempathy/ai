...
def main(args):
    ...
    for file in args.file:
        if os.path.splitext(file)[-1].lower() in image_ext:
            file = image_processor.preprocess(file, return_tensors="pt")["pixel_values"][0].to(
                model.device, dtype=torch.float16
            )
            special_token += [DEFAULT_IMAGE_TOKEN]
        elif os.path.splitext(file)[-1].lower() in video_ext:
            file = video_processor(file, return_tensors="pt")["pixel_values"][0].to(
                model.device, dtype=torch.float16
            )
    ...
    while True:
        ...
        if file is not None:
            ...
            if getattr(model.config, "mm_use_im_start_end", False):
                inp = (
                    "".join(
                        [DEFAULT_IM_START_TOKEN + i + DEFAULT_IM_END_TOKEN for i in special_token]
                    )
                    + "\n"
                    + inp
                )
