...
class MplugOwlVisionLocalTemporal(nn.Module):
    def __init__(self, config):
        ...
        self.down_proj = nn.Conv3d(
            self.hidden_size, d_bottleneck, kernel_size=1, stride=1, padding=0
        )
        self.conv = nn.Conv3d(
            d_bottleneck,
            d_bottleneck,
...
class MplugOwlVisionModel(MplugOwlPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=BaseModelOutputWithPooling, config_class=MplugOwlVisionConfig
    )
    def forward(
        self,
...
class MplugOwlForConditionalGeneration(MplugOwlPreTrainedModel):
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        media_token_types = [
            get_media_types(input_ids[i], media_token_indices[i]) for i in range(batch_size)
        ]
        ...
        if video_pixel_values is not None:
            ...
            with torch.no_grad():
                video_embeds = self.vision_model(
                    video_pixel_values, return_dict=True
                ).last_hidden_state
                video_attention_mask = torch.ones(
                    video_embeds.size()[:-1], dtype=torch.long, device=video_embeds.device
                )
                video_attention_mask = einops.rearrange(video_attention_mask, "b t n -> b (t n)")
                query_tokens = self.query_tokens.expand(video_embeds.shape[0], -1, -1)
                temporal_query_tokens = self.temporal_query_tokens.expand(
        ...
        for b in range(batch_size):
            ...
            for i, pos in enumerate(media_token_indices[b]):
                ...
                if media_token_types[b][i] == -1:
                    ...
                    result_attn.append(
                        torch.ones(
                            image_embeds[img_idx + curr_image_idx].shape[0],
                    ...
                    result_attn.append(
                        torch.ones(
                            video_embeds[img_idx + curr_video_idx].shape[0],
                            device=inputs_embeds.device,
                        )
                    )
