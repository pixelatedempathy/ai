    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                image = [
                    Image.open(os.path.join(image_folder, file.replace("\\", "/"))).convert("RGB")
            ...
            processor = self.data_args.image_processor
            image, _ = _get_rawvideo_dec(
                os.path.join(video_folder, video_file), processor, max_frames=MAX_IMAGE_LENGTH
            )
            sources = preprocess_multimodal(
                copy.deepcopy([e["conversations"] for e in sources]), self.data_args
            )
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.mm_use_box_start_end = data_args.mm_use_box_start_end = (
            model_args.mm_use_box_start_end
        )
    ...
    trainer = ChatUniViTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
