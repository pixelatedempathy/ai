...
class Chat:
    def __init__(self, model_path, conv_mode="simple"):
        ...
        self.tokenizer, self.model, self.image_processor, context_len = load_pretrained_model(
            model_path, None, model_name="ChatUniVi"
        )
    ...
    def generate(self, images_tensor: list, prompt: str, first_run: bool, state):
        ...
        stop_str = (
            conv_templates[self.conv_mode].copy().sep
            if conv_templates[self.conv_mode].copy().sep_style != SeparatorStyle.TWO
            else conv_templates[self.conv_mode].copy().sep2
        )
        keywords = [stop_str]
