...
class SALMONN(nn.Module):
    def __init__(
        ...
        if not low_resource:
            ...
            self.llama_model = LlamaForCausalLM.from_pretrained(
                vicuna_path, torch_dtype=torch.float16, load_in_8bit=True, device_map={"": 0}
            )
    ...
    def generate(
        ...
        speech_embeds_tr = speech_embeds.transpose(1, 2).unsqueeze(
            2
        )  # [batch=1, dim=2048, 1, seqlen=1500]
        speech_embeds_overlap = F.unfold(
            speech_embeds_tr, kernel_size=kernel, dilation=1, padding=0, stride=stride
        )  # 按照1D conv的格式展开为 [batch=1, dim=2048*1*17, seqlen=1500/17]
        ...
        speech_embeds = speech_embeds_overlap.reshape(
            -1, kernel[1], C
        )  # [1500/17=88, 17, 2048] ?? 感觉跟reshape没啥区别
        ...
        )  # 词典 Embedding(32000, 5120, padding_idx=0)
        prompt_left, prompts_right = prompt_pattern.format(prompt).split(
            "<SpeechHere>"
        )  # USER: <Speech><SpeechHere></Speech> {}\nASSISTANT: -> 分成左右两半
        prompt_left_ids = (
            self.llama_tokenizer(prompt_left, return_tensors="pt", add_special_tokens=False)
            .to(speech_embeds.device)
            .input_ids
        )
        prompt_left_embeds = embed_tokens(prompt_left_ids)
        prompt_right_ids = (
            self.llama_tokenizer(prompts_right, return_tensors="pt", add_special_tokens=False)
            .to(speech_embeds.device)
            .input_ids
        )
