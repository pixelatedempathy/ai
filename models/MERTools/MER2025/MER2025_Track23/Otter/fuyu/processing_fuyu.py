...
def _segment_prompt_into_text_token_conversions(prompt: str) -> List:
    ...
    for i, elem in enumerate(prompt_split):
        ...
        prompt_text_list.append(
            (
                elem,
                i > 1 and prompt_split[i - 1] in [TOKEN_BBOX_OPEN_STRING, TOKEN_POINT_OPEN_STRING],
            )
        )
...
class FuyuProcessor(ProcessorMixin):
    ...
    def _right_pad_inputs_with_attention_mask(
                ...
                if key == "input_ids":
                    ...
                    attention_mask = torch.cat(
                        [
                            ...
                            torch.zeros(tensor.shape[0], num_padding_tokens, dtype=torch.long),
                        ],
                        dim=1,
                    )
    ...
    def get_sample_encoding(
        ...
        max_seq_len_batch = min(
            max_prompt_length + self.max_tokens_to_generate, self.max_position_embeddings
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        batch_encoding = self._right_pad_inputs_with_attention_mask(
    ...
    def post_process_box_coordinates(self, outputs, target_sizes=None):
        ...
        def tokens_to_boxes(tokens, original_size):
            while (
                ...
                replacement = (
                    f" {TEXT_REPR_BBOX_OPEN}{top}, {left}, {bottom}, {right}{TEXT_REPR_BBOX_CLOSE}"
                )
        ...
        if target_sizes is None:
            target_sizes = (
                (self.image_processor.size["height"], self.image_processor.size["width"]),
            ) * len(outputs)
