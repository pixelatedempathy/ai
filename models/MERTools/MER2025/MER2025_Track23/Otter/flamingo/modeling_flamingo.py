...
class FlamingoPerceiverResampler(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.frame_embs = (
            nn.Parameter(torch.randn(max_num_frames, dim)) if exists(max_num_frames) else None
        )
        ...
        for _ in range(depth):
            self.layers.append(
                FlamingoPerceiverBlock(dim=dim, dim_head=dim_head, heads=heads, mult=ff_mult)
            )
...
class FlamingoModel(FlamingoPreTrainedModel):
    ...
    def __init__(
        ...
        if "llama" not in config.text_config._name_or_path:
            if config.text_config.architectures[0] == "MPTForCausalLM":
                ...
                text_tokenizer = AutoTokenizer.from_pretrained(
                    "mosaicml/mosaic-llama-redpajama-final-candidate"
                )
        ...
        self.cross_attn_every_n_layers = (
            config.cross_attn_every_n_layers if hasattr(config, "cross_attn_every_n_layers") else 4
        )
    ...
    def init_weights(self):
        ...
        if "lora_config" in self.config.__dict__:
            print(
                f"LoRA trainable param: {(sum(p.numel() for p in self.lang_encoder.parameters() if p.requires_grad)) / 1e9:.3f} B"
            )
        ...
        # print model size in billions of parameters in 2 decimal places
        print(
            f"Total Trainable param: {(sum(p.numel() for p in self.parameters() if p.requires_grad)) / 1e9:.3f} B"
        )
...
class FlamingoForConditionalGeneration(FlamingoPreTrainedModel):
    ...
    def init_weights(self):
        ...
        if not (
            "train_full_model" in self.config.__dict__ and self.config.train_full_model is True
        ):
        ...
        if (
            ...
            and self.config.train_vision_encoder is True
        ):
        ...
        if "lora_config" in self.config.__dict__:
            ...
            master_print(
                f"LoRA trainable param: {(sum(param.numel() for name, param in self.lang_encoder.named_parameters() if 'lora' in name)) / 1e6:.3f} M"
            )
            for name, param in self.lang_encoder.named_parameters():
                if "lora" in name:
                    param.requires_grad = True
