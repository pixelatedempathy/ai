...
def add_noise_to_video(video_path, blur_rate, save_path):
    ...
    videoWriter = cv2.VideoWriter(
        save_path, cv2.VideoWriter_fourcc("D", "I", "V", "X"), fps, (int(width), int(height)), True
    )
    while True:
        ret, frame = cap.read()
        if not ret:
...
def split_audio_from_video(video_path, audio_path):
    cmd = '%s -i "%s" -ac 1 -ar 16000 -loglevel quiet -y "%s"' % (
...
def merge_audio_and_video(audio_path, video_path, save_path):
    cmd = (
        '%s -i "%s" -i "%s" -vcodec copy -acodec copy -c:v copy -c:a aac -loglevel quiet -y "%s"'
        % (config.PATH_TO_FFMPEG, audio_path, video_path, save_path)
    )
    os.system(cmd)
...
def func_mixture(
    ...
    info_a_noise = add_noise_to_audio(
        save_path1, noise_paths, snr, save_path2
    )  # add noise to audio
    ...
    os.system(
        'rm -rf "%s" "%s" "%s"' % (save_path1, save_path2, save_path3)
