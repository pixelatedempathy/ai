...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="attention type for spatiotemporal modeling",
    )
    ### factorised
    parser.add_argument(
        "--fst_share_st_attn",
    ...
    parser.add_argument(
        "--tf_num_tokens", type=int, default=8, help="number of tokens for token fuser"
    )
    parser.add_argument(
        "--tf_bottleneck_dim", type=int, default=64, help="bottleneck dim for token fuser"
    )
    ### perceiver
    parser.add_argument("--p_start_layer", type=int, default=7, help="start layer for perceiver")
    parser.add_argument(
        "--p_num_latents", type=int, default=16, help="number of latents for perceiver"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="number of self-attn layers in perceiver block",
    )
    parser.add_argument("--p_dim", type=int, default=768, help="hidden dim for perceiver")
    parser.add_argument(
        "--p_num_cross_heads", type=int, default=4, help="number of cross-attn heads for perceiver"
    )
    ### part window
    parser.add_argument(
        "--part_win_size",
        ...
        nargs="+",
        default=(2, 2, 10),
        help="window size (t,h,w) for part window attn",
    )
    parser.add_argument(
        "--part_cls_type", type=str, default="org", help="classification type for part window attn"
    )
    parser.add_argument(
        "--part_local_first",
        action="store_true",
        help="perform global part attn after local window attn for part window attn",
    )
    ### temporal window
    parser.add_argument(
        "--tem_win_size",
    ...
    parser.add_argument(
        "--tem_pyr_stride", type=int, default=2, help="stride for temporal pyramid downsample"
    )
    parser.add_argument(
        "--tem_pyr_type_up",
        type=str,
    ...
    parser.add_argument(
        "--tem_pyr_no_use_multiscale_feature",
        action="store_true",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=(8,),
        help="number of tokens for spatial downsampling in spatial temporal pyramid attention",
    )
    parser.add_argument(
        "--st_tf_bottleneck_dim",
        type=int,
        default=128,
        help="bottleneck dim for spatial downsampling in spatial temporal pyramid attention",
    )
    parser.add_argument(
        "--st_pyr_no_multiscale_feature",
        action="store_true",
        help="do not use multiscale features for final classification",
    )
    parser.add_argument(
        "--st_pyr_spatial_only_cross_attn",
        action="store_true",
        help="only use cross attn for spatial upsample in spatial temporal pyramid attention",
    )
    parser.add_argument(
        "--st_pyr_window_size",
        type=int,
        nargs="+",
        default=None,
        help="window size for applying 3d shifted window attention in the first stage",
    )
    parser.add_argument(
        "--st_pyr_disable_temporal_pyr",
        action="store_true",
        help="do not use temporal pyramid in spatial temporal pyramid attention",
    )
    parser.add_argument(
        "--st_pyr_disable_spatial_pyr",
        action="store_true",
        help="do not use spatial pyramid in spatial temporal pyramid attention",
    )
    ### local_global
    parser.add_argument(
        "--lg_region_size",
    ...
    # Dataset parameters
    parser.add_argument(
        "--data_path", default="/path/to/list_kinetics-400", type=str, help="dataset path"
    )
    parser.add_argument(
        "--eval_data_path", default=None, type=str, help="dataset path for evaluation"
    )
    parser.add_argument(
        "--nb_classes", default=6, type=int, help="number of the classification types"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="acc1",
        choices=["acc1", "acc5", "war", "uar", "weighted_f1", "micro_f1", "macro_f1"],
...
def main(args):
    ...
    save_dir = os.path.join(
        config.PATH_TO_FEATURES[args.dataset], f"{model_name}-{args.feature_level[:3]}"
    )
    ...
    model = create_model(
        ...
        p_start_layer=args.p_start_layer,
        p_num_latents=args.p_num_latents,
        p_num_layer=args.p_num_layer,  # for perceiver
        p_num_latent_layer=args.p_num_latent_layer,
        p_dim=args.p_dim,
        p_num_cross_heads=args.p_num_cross_heads,
        part_win_size=args.part_win_size,
        part_cls_type=args.part_cls_type,
        part_local_first=args.part_local_first,  # for part window attention
        tem_win_size=args.tem_win_size,
        tem_win_depth=args.tem_win_depth,  # for temporal window attention
        tem_pyr_depth=args.tem_pyr_depth,
        tem_pyr_kernel_size=args.tem_pyr_kernel_size,  # for temporal pyramid
        tem_pyr_stride=args.tem_pyr_stride,
        tem_pyr_type=args.tem_pyr_type,
        tem_pyr_type_up=args.tem_pyr_type_up,
        tem_pyr_no_use_multiscale_feature=args.tem_pyr_no_use_multiscale_feature,
        st_pyr_depth=args.st_pyr_depth,
        st_pyr_kernel_size=args.st_pyr_kernel_size,
        st_pyr_stride=args.st_pyr_stride,  # for spatial temporal pyramid
        st_pyr_type=args.st_pyr_type,
        st_pyr_type_up=args.st_pyr_type_up,
        st_tf_num_tokens=args.st_tf_num_tokens,
        st_tf_bottleneck_dim=args.st_tf_bottleneck_dim,
        ...
        lg_first_attn_type=args.lg_first_attn_type,  # for local_global
