...
class VideoLLAMA(Blip2Base):
    ...
    def __init__(
        ...
        if (frozen_audio_Qformer and frozen_audio_llama_proj) and (
            not frozen_video_Qformer or not frozen_llama_proj
        ):
            self.train_flag = 1  # training on video branch
        elif (frozen_video_Qformer and frozen_llama_proj) and (
            not frozen_audio_Qformer or not frozen_audio_llama_proj
        ):
            self.train_flag = 2  # training on audio branch
        elif (not frozen_video_Qformer or not frozen_llama_proj) and (
            not frozen_audio_Qformer or not frozen_audio_llama_proj
        ):
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision, vit_path
        )
        ...
        self.video_frame_position_embedding = nn.Embedding(
            32, self.Qformer.config.hidden_size
        )  # [32, 768]
    ...
    def encode_audioQformer(self, audio, modality_type=ModalityType.AUDIO):
        ...
        with self.maybe_autocast():
            ...
            frame_atts = torch.ones(audio_imagebind_finalout.size()[:-1], dtype=torch.long).to(
                device
            )
    ...
    def forward(self, samples):
        ...
        if "images" in samples:
            img_embeds, _ = self.encode_videoQformer_visual(
                samples["images"]
            )  # image: [b c t h w] -> [b, 32, 4096]
        if "audios" in samples:
            aud_embeds, _ = self.encode_audioQformer(
                samples["audios"]
        ...
        for cur_input_ids, cur_input_embeds in zip(input_ids, temp_input_embedding):
            ...
            if (cur_input_ids == im_patch_token_id).sum() != 0:
                ...
                if (
                    ...
                    != torch.arange(
                        mask_index_start,
                        mask_index_start + self.num_video_query_token,
                ...
                cur_input_embeds = torch.cat(
                    (
                        ...
                        cur_image_features,
                        cur_input_embeds[mask_index_start + self.num_video_query_token :],
                    ),
                    dim=0,
                )
            ...
            if (cur_input_ids == au_patch_token_id).sum() != 0:
                ...
                if (cur_input_ids == au_patch_token_id).sum() != self.num_audio_query_token:
                    raise ValueError(
                        "The number of image patch tokens should be the same as the number of audio patches."
                    )
                ...
                if (
                    ...
                    != torch.arange(
                        mask_index_start,
                        mask_index_start + self.num_audio_query_token,
                        device=masked_indices.device,
                        dtype=masked_indices.dtype,
                    )
                ).any():
                ...
                cur_input_embeds = torch.cat(
                    (
                        cur_input_embeds[:mask_index_start],
                        cur_audio_features,
                        cur_input_embeds[mask_index_start + self.num_audio_query_token :],
                    ),
                    dim=0,
                )
        ...
        with self.maybe_autocast():
            ...
            outputs = outputs * torch.unsqueeze(
                attention_mask, dim=2
            )  # [batch, seqlen, dim] * [batch, seqlen, 1] -> sum 压缩到 [batch, dim]
            features = torch.sum(outputs, dim=1) / torch.sum(
