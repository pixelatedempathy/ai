        ...
        for slot_name, slot in domain["slots"].items():
            ...
            if slot["is_categorical"]:
                ...
                for value in special_values:
                    assert value not in slot["possible_values"], (
...
def check_dialogues(name, dialogues, ontology):
    ...
    stat_keys = [
        ...
        "tokens",
        "domains",
        "cat slot match(state)",
        "cat slot match(goal)",
        "cat slot match(dialogue act)",
        "non-cat slot span(dialogue act)",
    ]
    stat = {
        split: {key: 0 if "slot" not in key else [0, 0] for key in stat_keys} for split in splits
    }
    ...
    for dialogue in dialogues:
        ...
        assert isinstance(dialogue_id, str), (
            f"{dialogue_id}\t`dialogue_id` is expected to be str type"
        )

        assert dialogue["dataset"] == name, (
            f"{dialogue_id}\tinconsistent dataset name: {dialogue['dataset']}"
        )

        split = dialogue["data_split"]
        assert isinstance(split, str), (
            f"{dialogue_id}\t`split` is expected to be str type but got {type(split)}"
        )
        ...
        if "domains" in dialogue:
            ...
            assert isinstance(cur_domains, list), (
        ...
        # prefix: error prefix
        def check_dsv(
            domain_name, slot_name, value, anno_type, categorical=None, prefix=f"{dialogue_id}"
        ):
            if anno_type != "state":
                assert domain_name in cur_domains, (
                    f"{prefix}\t{domain_name} not presented in current domains"
                )
            domain = ontology["domains"][domain_name]
            assert slot_name in domain["slots"], (
                f"{prefix}\t{slot_name} not presented in domain {domain_name} in ontology"
            )
            ...
            if categorical and len(value) > 0:
                for v in value.split("|"):
                    ...
                    if v in special_values or v.lower() in [
                        s.lower() for s in slot["possible_values"]
                    ]:
        ...
        def check_da(da, categorical):
            assert da["intent"] in ontology["intents"], (
        ...
        if "goal" in dialogue:
            ...
            assert isinstance(goal["description"], str), (
                f"{dialogue_id}\tgoal description {goal['description']} should be string"
            )
            assert isinstance(goal["inform"], dict), (
                f"{dialogue_id}\tgoal inform {goal['inform']} should be dict"
            )
            assert isinstance(goal["request"], dict), (
                f"{dialogue_id}\tgoal request {goal['request']} should be dict"
            )
            for domain_name, domain in goal["inform"].items():
                for slot_name, value in domain.items():
                    check_dsv(
                        domain_name, slot_name, value, "goal", prefix=f"{dialogue_id}:goal:inform"
                    )
                    assert value != "", (
                        f"{dialogue_id}\tshould set non-empty value in goal inform {goal['inform']}"
                    )
            for domain_name, domain in goal["request"].items():
                for slot_name, value in domain.items():
                    check_dsv(
                        domain_name, slot_name, value, "goal", prefix=f"{dialogue_id}:goal:request"
                    )
                    assert value == "", (
                        f"{dialogue_id}\tshould set empty value in goal request {goal['request']}"
                    )
        ...
        for turn_id, turn in enumerate(turns):
            ...
            if "dialogue_acts" in turn:
                ...
                assert isinstance(dialogue_acts["categorical"], list), (
                    f"{dialogue_id}:{turn_id}\tcategorical dialogue_acts should be a list"
                )
                assert isinstance(dialogue_acts["non-categorical"], list), (
                    f"{dialogue_id}:{turn_id}\tnon-categorical dialogue_acts should be a list"
                )
                assert isinstance(dialogue_acts["binary"], list), (
                    f"{dialogue_id}:{turn_id}\tbinary dialogue_acts should be a list"
                )
                ...
                for da in dialogue_acts["non-categorical"]:
                    ...
                    if da["value"] not in special_values:
                        stat[split]["non-cat slot span(dialogue act)"][1] += 1
                        ...
                        if "start" in da:
                            ...
                            assert da["value"] == value, (
                                f"{dialogue_id}:{turn_id}\tspan({value}) and value{da['value']} not match"
                            )
                            stat[split]["non-cat slot span(dialogue act)"][0] += 1
                    ...
                    for da in dialogue_acts[da_type]:
                        ...
                        if da_type == "binary":
                            assert "value" not in da, (
                                f"{dialogue_id}:{turn_id}\tbinary dialogue act should not have value"
                            )
            ...
            if turn["speaker"] == "user":
                ...
                if "state" in turn:
                    ...
                    assert isinstance(state, dict), (
                        f"{dialogue_id}:{turn_id}\tstate should be a dict"
                    )
                    for domain_name, domain in state.items():
                        for slot_name, value in domain.items():
                            check_dsv(
                                ...
                                value,
                                "state",
                                prefix=f"{dialogue_id}:{turn_id}:state",
                            )

            else:
                assert "state" not in turn, (
                    f"{dialogue_id}:{turn_id}\tstate cannot present in system's role"
                )
                if "db_results" in turn:
                    db_results = turn["db_results"]
                    assert isinstance(db_results, dict), (
                        rf"{dialogue_id}:{turn_id}\db_results should be a dict"
                    )
    ...
    for _, value_match in match_rate.items():
        for anno_type, (match, total) in value_match.items():
            if total == 0:
                ...
                value_match[anno_type] = f"{match * 100 / total:.3f}"
    ...
    for split in splits + ["all"]:
        ...
        if cur_stat["dialogues"]:
            ...
            table.append(
                {
                    "split": split,
                    "dialogues": cur_stat["dialogues"],
                    "utterances": cur_stat["utterances"],
                    "avg_utt": cur_stat["avg_utt"],
                    "avg_tokens": cur_stat["avg_tokens"],
                    "avg_domains": cur_stat["avg_domains"],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "datasets", metavar="dataset_name", nargs="*", help="dataset names to be tested"
    )
        ...
        try:
            ...
            with ZipFile(data_file) as zipfile:
                ...
                with zipfile.open("data/dialogues.json", "r") as f:
                    ...
                    with open(
                        os.path.join(name, "shuffled_dial_ids.json"), "w", encoding="utf-8"
                    ) as f:
                        json.dump(id_orders, f, ensure_ascii=False)

                print(
                    f"Please copy and paste the statistics in {name}/stat.txt to dataset README.md->Data Splits section\n"
                )
