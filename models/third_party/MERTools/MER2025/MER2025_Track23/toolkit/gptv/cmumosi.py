                ...
                for newbatch, newsave in newstores:
                    ...
                    for new2batch, new2save in new2stores:
                        if os.path.exists(new2save):
                            continue
                        response = func_get_response(
                            new2batch, emos, modality, sleeptime, samplenum
                        )
...
def get_results_and_update_label(gpt4v_root, label_path, store_path):
    ...
    non_zeros = np.array(
        [i for i, e in enumerate(test_labels) if e != 0]
    )  # remove 0, and remove mask
...
def further_report_results(gpt4v_csv):
    ...
    for repeat in range(10):  # remove randomness
        ...
        fscore = f1_score(
            (test_labels[non_zeros] > 0), (test_preds[non_zeros] > 0), average="weighted"
        )
