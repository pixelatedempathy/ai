...
class MetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "config"):
            ...
            if self.use_cluster:
                self.ctm0 = CTM(
                    sample_ratio=config.config["spatial_cluster_rate0"],
                    embed_dim=self.config.mm_hidden_size,
                    dim_out=self.config.mm_hidden_size,
                    k=5,
                )
                self.block0 = TCBlock(dim=self.config.mm_hidden_size, num_heads=8)

                self.ctm1 = CTM(
                    sample_ratio=config.config["spatial_cluster_rate1"],
                    embed_dim=self.config.mm_hidden_size,
                    dim_out=self.config.mm_hidden_size,
                    k=3,
                )
                ...
                self.ctm2 = CTM(
                    sample_ratio=config.config["spatial_cluster_rate2"],
                    embed_dim=self.config.mm_hidden_size,
                    dim_out=self.config.mm_hidden_size,
                    k=3,
                )
                ...
                self.ctm3 = CTM(
                    sample_ratio=config.config["temporal_cluster_rate"],
                    embed_dim=self.config.mm_hidden_size,
                    dim_out=self.config.mm_hidden_size,
                    k=5,
                )
    ...
    def initialize_cluster_modules(self, model_args):
        ...
        if self.use_cluster and not hasattr(self, "ctm0"):
            self.ctm0 = CTM(
                sample_ratio=model_args.spatial_cluster_rate0,
                embed_dim=self.config.mm_hidden_size,
                dim_out=self.config.mm_hidden_size,
                k=5,
            )
            ...
            self.ctm1 = CTM(
                sample_ratio=model_args.spatial_cluster_rate1,
                embed_dim=self.config.mm_hidden_size,
                dim_out=self.config.mm_hidden_size,
                k=3,
            )
            ...
            self.ctm2 = CTM(
                sample_ratio=model_args.spatial_cluster_rate2,
                embed_dim=self.config.mm_hidden_size,
                dim_out=self.config.mm_hidden_size,
                k=3,
            )
            ...
            self.ctm3 = CTM(
                sample_ratio=model_args.temporal_cluster_rate,
                embed_dim=self.config.mm_hidden_size,
                dim_out=self.config.mm_hidden_size,
                k=5,
            )
        ...
        if self.get_model().use_cluster:
            if input_type == "image":
                ...
                token_dict = {
                    ...
                    "agg_weight": image_features.new_ones(
                        image_features.size(0), image_features.size(1), 1
                    ),
                    "mask": None,
                }
                ...
                token_dict = {
                    ...
                    "agg_weight": cls_features.new_ones(
                        cls_features.size(0), cls_features.size(1), 1
                    ),
                    "mask": None,
                }
                ...
                for id, key in enumerate(events):
                    cur_image_features0 = torch.cat(
                        [token_dict0["x"][i] for i in events[key]], dim=0
                    ).unsqueeze(0)
                    token_dict = {
                        ...
                        "agg_weight": cur_image_features0.new_ones(
                            cur_image_features0.size(0), cur_image_features0.size(1), 1
                        ),
                        "mask": None,
                    }
                    ...
                    cur_image_features1 = torch.cat(
                        [token_dict1["x"][i] for i in events[key]], dim=0
                    ).unsqueeze(0)
                    token_dict = {
                        ...
                        "agg_weight": cur_image_features1.new_ones(
                            cur_image_features1.size(0), cur_image_features1.size(1), 1
                        ),
                        "mask": None,
                    }
                    ...
                    cur_image_features2 = torch.cat(
                        [token_dict2["x"][i] for i in events[key]], dim=0
                    ).unsqueeze(0)
                    token_dict = {
                        "x": cur_image_features2,
                        ...
                        "agg_weight": cur_image_features2.new_ones(
                            cur_image_features2.size(0), cur_image_features2.size(1), 1
                        ),
                        "mask": None,
                    }
            ...
            if input_type == "video":
                image_features, cls_features = torch.mean(
                    image_features, dim=0, keepdim=False
                ).unsqueeze(0), torch.mean(image_features, dim=1, keepdim=False).unsqueeze(0)
            ...
            if len(image_token_indices) > 1:
                ...
                for i in temp:
                    ...
                    if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                        ...
                        cur_new_input_embeds.append(
                            self.get_model().embed_tokens(
                                cur_input_ids[image_token_end + 1 : image_token_end + 2]
                            )
                        )
                        if labels is not None:
