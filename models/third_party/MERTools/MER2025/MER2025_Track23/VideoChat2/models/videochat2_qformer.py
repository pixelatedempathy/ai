...
class VideoChat2_qformer(nn.Module):
    ...
    def vtm_loss(
        ...
        text_ids_pos_neg = torch.cat([text.input_ids, text.input_ids, text_ids_neg], dim=0)
    ...
    def vtm_embed(self, text_ids, text_atts, vision_embeds, vision_atts):
        ...
        if self.config.criterion.get("vtm_add_text_cls", False):
            ...
            vl_embeddings = torch.cat(
                [tmp_embeddings, output_itm.last_hidden_state[:, query_tokens.size(1)]], dim=1
            )
        else:
