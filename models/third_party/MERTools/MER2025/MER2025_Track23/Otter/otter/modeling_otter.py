    ...
    if not XFORMERS_MSG_PRINTED:  # Check if the message has been master_printed before
        ...
        if dist.is_initialized() and dist.get_rank() == 0:  # Check if the current process rank is 0
            ...
            XFORMERS_MSG_PRINTED = (
                True  # Set the variable to True after master_printing the message
            )
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(depth):
            self.layers.append(
                OtterPerceiverBlock(dim=dim, dim_head=dim_head, heads=heads, mult=ff_mult)
            )
...
class OtterModel(OtterPreTrainedModel):
    ...
    def __init__(
        ...
        self.perceiver = OtterPerceiverResampler(
            dim=self.vis_dim, max_num_frames=self.max_num_frames
        )
    ...
    def init_weights(self):
        ...
        master_print(
            f"Trainable param: {(sum(p.numel() for p in self.parameters() if p.requires_grad)) / 1e9:.2f} B"
        )
