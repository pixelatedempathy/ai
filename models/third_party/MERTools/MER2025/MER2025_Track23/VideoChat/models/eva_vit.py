...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, temporal_downsample=False
    ):
        super().__init__()
        ...
        if temporal_downsample:
            self.proj = nn.Conv3d(
                in_chans,
                embed_dim,
                kernel_size=(3, patch_size[0], patch_size[1]),
                stride=(2, patch_size[0], patch_size[1]),
                padding=(1, 0, 0),
            )
        else:
            self.proj = nn.Conv3d(
...
class Global_MHRA(nn.Module):
    def __init__(
        ...
        self.dpe = nn.Conv3d(
            d_model, d_model, kernel_size=3, stride=1, padding=1, bias=True, groups=d_model
        )
        nn.init.constant_(self.dpe.bias, 0.0)
        ...
        self.gmhra = nn.ModuleList(
            [
                Global_MHRA(
                    ...
                    drop_path=gmhra_dpr[i],
                    dropout=gmhra_dropout,
                )
