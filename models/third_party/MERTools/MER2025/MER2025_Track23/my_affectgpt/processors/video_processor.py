...
def load_video(
    video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg=False
):
    decord.bridge.set_bridge("torch")
    ...
    # get_batch -> T, H, W, C
    temp_frms = vr.get_batch(
        indices
    )  # 这块报错 [h264 @ 0xc97e880] mmco: unref short failure => 这通常的是视频本身的问题
