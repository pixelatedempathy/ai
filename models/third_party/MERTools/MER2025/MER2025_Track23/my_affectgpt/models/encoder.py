...
class EVA_CLIP_G(Blip2Base):
    def __init__(self):
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            config.PATH_TO_VISUAL["EVA_CLIP_G"],
            self.img_size,
            self.drop_path_rate,
...
class EVA_CLIP_G_NO_QFORMER(Blip2Base):
    ...
    def forward(self, image, raw_image):
        ...
        image_embeds = einops.rearrange(
            image_embeds, "(b t) h -> b t h", b=batch_size, t=time_length
        )
...
class DINO2_LARGE(Blip2Base):
    ...
    def forward(self, image, raw_image):
        ...
        ).hidden_states  # [(b, t)] * [58, 196 patch + 1 cls, feat=768]
        embeddings = torch.stack(embeddings)[-1].mean(
            dim=1
        )  # 读取最后一层特征, 所有block特征取平均, [(b t), feat=768]
...
class SigLIP_SO(Blip2Base):
    ...
    def forward(self, image, raw_image):
        ...
        inputs = self.processor(images=raw_image, return_tensors="pt")["pixel_values"]
        inputs = inputs.to(device)
        embeddings = self.model.vision_model(
            inputs, output_hidden_states=True
        ).hidden_states  # [(b, t)] * [58, 196 patch + 1 cls, feat=768]
        embeddings = torch.stack(embeddings)[-1].mean(
            dim=1
...
class IMAGEBIND(Blip2Base):
    ...
    def forward(self, audio, raw_audio):
        device = audio.device
        _, embeddings = self.audio_encoder.get_audio_feature(
...
class DATA2VEC_BASE(Blip2Base):
    ...
    def forward(self, audio, raw_audio):
        ...
        hidden_states = self.model(
            input_values[0], output_hidden_states=True
        ).hidden_states  # tuple of ((b t), T, D)
