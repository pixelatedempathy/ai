        ...
        def _create_loader(dataset, num_workers, bsz, is_train, collate_fn):
            ...
            if isinstance(dataset, ChainDataset) or isinstance(dataset, wds.DataPipeline):
                ...
                loader = DataLoader(
                    ...
                    and is_train,  # 默认是 False，因为已经在 sampler 内部采样了
    ...
    def _save_checkpoint(self, cur_epoch, train_stats=None, is_best=False):
        ...
        if is_best:
            ...
            save_to = os.path.join(
                self.output_dir, "checkpoint_%06d_loss_%s.pth" % (cur_epoch, "0.000")
            )
        else:
            save_to = os.path.join(
                self.output_dir, "checkpoint_%06d_loss_%s.pth" % (cur_epoch, train_stats["loss"])
            )
