        ...
        if labels is not None:
            ...
            if mem is not None and mem.get("pos", None) is not None:
                ...
                sigma_pos = torch.mean(
                    torch.bmm((pos_all - mu_pos).unsqueeze(-1), (pos_all - mu_pos).unsqueeze(1)),
                    dim=0,
                )
                sigma_neg = torch.mean(
                    torch.bmm((neg_all - mu_neg).unsqueeze(-1), (neg_all - mu_neg).unsqueeze(1)),
                    dim=0,
                )
                a = 17.0795
...
class MMIM(nn.Module):
    def __init__(self, args):
        ...
        # For MI maximization
        self.mi_tv = MMILB(
            x_size=hidden_dim, y_size=hidden_dim, mid_activation="ReLU", last_activation="Tanh"
        )
        ...
        self.cpc_zv = CPC(
            x_size=hidden_dim, y_size=hidden_dim, n_layers=cpc_layers, activation="Tanh"
        )
        self.cpc_za = CPC(
            x_size=hidden_dim, y_size=hidden_dim, n_layers=cpc_layers, activation="Tanh"
        )
        ...
        # Trimodal Settings
        self.fusion_prj = Fusion(in_size=hidden_dim * 3, hidden_size=hidden_dim, dropout=dropout)
