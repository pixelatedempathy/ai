...
class OPENSMILE(object):
    def __init__(self, path):
        ...
        if sys.platform == "win32":
            ...
            self.OPENSMILE_EXE = os.path.join(
                self.path, r"bin/linux_x64_standalone_static/SMILExtract"
            )
        ...
        self.FrameModeFunctionals_CFG_FILE = os.path.join(
            self.path, r"config/shared/FrameModeFunctionals.conf.inc"
        )

    def _verify_feature_level(self, feature_level):
        assert feature_level.upper() in ["FRAME", "UTTERANCE"], (
            "Invalid acoustic feature level: %s!" % feature_level
        )
    ...
    def extract_acoustic_feature(
        ...
        if level == "FRAME":  # frame level feature
            ...
            if frame_mode_functionals_param is not None:
                ...
                cmd = template.format(
                    self.OPENSMILE_EXE, cfg_file, input_file, input_file, output_file
                )
        # print(cmd)
    ...
    def parse_acoustic_feature_csv_file(self, csv_file, level):
        ...
        with open(csv_file, "r") as f:
            for line in f.readlines()[1:]:  # start from the 2nd row
                ...
                acoustic_feature.append(
                    line_feature
                )  # Note: features is a 2-dim list. (no matter frame feature or utterance feature)
...
class Librosa:
    ...
    @staticmethod
    def extract_acoustic_feature(
        input_file, feature_set, frame_size=0.025, frame_step=0.010, **kwargs
    ):
        if feature_set == "mel_spec":
            features = Librosa.extract_mel_spectrogram(
                input_file, frame_size=frame_size, frame_step=frame_step
            )
        elif feature_set == "mfcc":
            features = Librosa.extract_mfcc(
                input_file, frame_size=frame_size, frame_step=frame_step
            )
        else:
            raise Exception(
                f'Not supported feature set "{feature_set}" for audio feature extractor "Librosa".'
            )
        return features
    ...
    def extract_mel_spectrogram(
        ...
        mel_spec = librosa.feature.melspectrogram(
            y=y, sr=sr, win_length=win_length, hop_length=hop_length, n_fft=n_fft, n_mels=n_mels
        )  # "window" is 'hann' by default
        if log_mel:
            # 10*log10()
...
if __name__ == "__main__":
    ...
    feature = myOpensmile.extract_acoustic_feature(
        input_file=input_file,
        output_file=output_file,
        feature_set="IS09",
        feauture_level="UTTERANCE",
        frame_mode_functionals_param=(0.2, 0.1),
    )  # segment-level feature (win_len: 200ms, hop_len: 100ms)
    print(feature.shape)
