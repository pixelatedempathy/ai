...
def extract(audio_files, feature_level, model, save_dir, overwrite=False, gpu=None):
    ...
    out_dir_z = os.path.join(
        save_dir, f"{dir_name}-z-{feature_level[:3]}"
    )  # features output by feature encoder
    if not os.path.exists(out_dir_z):
        ...
        raise Exception(
            f'==> Error: save_dir "{save_dir}" already exists, set overwrite=TRUE if needed!'
        )

    out_dir_c = os.path.join(
        save_dir, f"{dir_name}-c-{feature_level[:3]}"
    )  # features output by context network
    ...
    for idx, wav_file in enumerate(audio_files, 1):
        ...
        with torch.no_grad():
            z = model.feature_extractor(
                audio
            )  # (1, C, T), stride: 10ms (100Hz), receptive field: 30ms
            c = model.feature_aggregator(
                z
            )  # (1, C, T), stride: 10ms (100Hz), receptive field: 801ms (for large version)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="FRAME",
        help="name of feature level, FRAME or UTTERANCE",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
