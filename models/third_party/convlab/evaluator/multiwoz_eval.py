...
class MultiWozEvaluator(Evaluator):
    def __init__(
        self, check_book_constraints=True, check_domain_success=False, action_length_penalty=0.0
    ):
        self.sys_da_array = []
        self.usr_da_array = []
        self.goal = {}
        ...
        for intent, domain, slot, value in da_turn:
            if intent.lower() == "book" and not value:
                ref = [
                    ...
                    if _domain == domain and _intent.lower() == "inform" and _slot.lower() == "ref"
                ]
    ...
    def task_success(self, ref2goal=True):
        ...
        if (
            ...
            self.complete = (
                1 if booking_done and (inform_sess[1] == 1 or inform_sess[1] is None) else 0
            )
            self.success = 0
            self.success_strict = 0
            return 0
            ...
            if intent.lower() in ["inform", "recommend"]:
                if domain.lower() in goal:
                    if "reqt" in goal[domain.lower()]:
                        if (
                            REF_SYS_DA_M.get(domain.lower(), {}).get(slot.lower(), slot.lower())
