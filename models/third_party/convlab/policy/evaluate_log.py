...
def plot_performance(log_paths, plot_success=True, labels=None, ppo=None):
    ...
    with sns.axes_style("darkgrid"):
        for i, path in enumerate(log_paths):
            epoch, complete_rate, success_rate, dialog_counter = extract_performance(
                path, ppo=ppo[i]
            )
            if plot_success:
            ...
            plt.plot(
                dialog_counter[: len(performance) - 1], performance[:-1], c=clrs[i], label=labels[i]
            )
        ...
        plt.ylabel(y_label)
        plt.legend(
            loc="upper center", bbox_to_anchor=(0.65, 0.35), fancybox=True, shadow=True, ncol=1
        )
        plt.show()


def plot_performance_across_seeds(
    log_dir_ensemble_pairs, plot_success=True, labels=None, plot_return=False, save_path=None
):
    best_performance = []
    ...
    with sns.axes_style("darkgrid"):
        for i, (direc, ensemble_num) in enumerate(log_dir_ensemble_pairs):
            ...
            for file in os.listdir(direc):
                if ensemble_num is False:
                    epoch, complete_rate, success_rate, dialog_counter, total_return = (
                        extract_performance(os.path.join(direc, file + "/log.txt"), ppo=True)
                    )
                    ...
                    if file.startswith(str(ensemble_num)):
                        epoch, complete_rate, success_rate, dialog_counter, total_return = (
                            extract_performance(os.path.join(direc, file + "/log.txt"), ppo=False)
                        )
                        complete_rate = complete_rate[: len(dialog_counter)]
            ...
            mean_success, err_success = np.mean(success_rate_across_seeds, axis=0), np.std(
                success_rate_across_seeds, axis=0
            )
            mean_return, err_return = np.mean(total_return_across_seeds, axis=0), np.std(
                total_return_across_seeds, axis=0
            )
        ...
        plt.ylabel(y_label)
        plt.legend(
            loc="upper center", bbox_to_anchor=(0.78, 0.72), fancybox=True, shadow=False, ncol=1
        )
