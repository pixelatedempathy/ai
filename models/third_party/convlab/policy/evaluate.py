...
def evaluate(
    config_path, model_name, verbose=False, model_path="", goals_from_data=False, dialogues=500
):
    seed = 0
    set_seed(seed)
    ...
    if goals_from_data:
        ...
        goals = create_goals(
            goal_generator, num_goals=dialogues, single_domains=False, allowed_domains=None
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="config path defining the environment for simulation and system pipeline",
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="",
        help="if this is set, tries to load the model weights from this path"
        ", otherwise from config",
    )
    parser.add_argument(
        "-N", "--num_dialogues", type=int, default=500, help="# of evaluation dialogue"
    )
    parser.add_argument("-V", "--verbose", action="store_true", help="whether to output utterances")
    parser.add_argument(
    ...
    parser.add_argument(
        "-D", "--goals_from_data", action="store_true", help="load goal from the dataset"
    )
