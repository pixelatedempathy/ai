...
def training_info(conversation):
    r = {"complete": [], "task_succ": [], "task_succ_strict": [], "sentiment": []}
...
def _training_info(conversation: dict):
    ...
    for seed, dialog in conversation.items():
        if "info" in dialog:
            if "All_user_sim" in dialog["info"]:
                # old version
                return {
                    "complete": np.average(dialog["info"]["All_user_sim"]),
    ...
    return {
        "complete": np.average(r["complete"]),
...
def plot(data: dict, folder: str, title: str = None):
    ...
    for m in ["complete", "task_succ", "task_succ_strict", "sentiment"]:
        ...
        for label, exp in data.items():
            ...
            ax.plot(x, mean, marker=marker, linestyle="--", color=exp["color"], label=label)
            ax.fill_between(x, mean + std, mean - std, color=exp["color"], alpha=0.1)
...
def merge_seeds(data):
    ...
    for e in r["x"]:
        for m in epochs[0]:
            ...
            r[m]["std"].append(np.std(epochs[e][m], ddof=1) / np.sqrt(len(epochs[e][m])))
    return r
...
class Table:
    ...
    def update(self, folder, data):
        ...
        for e in range(self.max_epoch + 1):
            if e < len(data):
                self.success[str(e)].append(np.mean(data[e]["task_succ_strict"]))
    ...
    def to_csv(self, path):
        ...
        pd.DataFrame(self.sentiment).to_csv(os.path.join(path, "sentiment.csv"))
        ...
        for exp in task_map["models"]:
            ...
            for experiment in ["experiments", "finished_experiments"]:
                for exp_folder in glob(os.path.join(folder, experiment, "*")):
                    temp = {}
                    path = os.path.join(exp_folder, "logs", "conversation", "*.json")
                    for epoch, file in enumerate(sorted(glob(path))):
                        conversation = json.load(open(file))
                        temp[epoch] = training_info(conversation["conversation"])
                    data.append(temp)
                    print(exp_folder)
