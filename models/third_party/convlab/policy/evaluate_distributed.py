...
def sample(sess, seedrange, process_num, goals):
    ...
    for i in range(process_num):
        process_args = (
            ...
            evt,
            sess,
            seedrange[i * num_seeds_per_thread : (i + 1) * num_seeds_per_thread],
...
def evaluate_distributed(sess, seed_range, process_num, goals):
    ...
    return (
        batch.complete,
        batch.success,
        batch.success_strict,
        batch.total_return_success,
        batch.turns,
        batch.avg_actions,
        batch.task_success,
        np.average(batch.book_actions),
        np.average(batch.inform_actions),
        np.average(batch.request_actions),
        np.average(batch.select_actions),
        np.average(batch.offer_actions),
        np.average(batch.recommend_actions),
        np.average(batch.emotion_return),
    )
