...
@pytest.mark.skipif(convert_to_chatml is None, reason="convert_to_chatml not implemented")
class TestChatMLConversion:
    def test_basic_conversion(self):
        """Given a DataFrame with 'prompt' and 'response', when converting, then output is valid ChatML format."""
    ...
    def test_missing_columns_raises(self):
        """Given a DataFrame missing required columns, when converting, then ValueError is raised."""
        df = pd.DataFrame([{"input": "foo"}])
        with pytest.raises(ValueError):
            convert_to_chatml(df)

    def test_multilingual_content(self):
        """Given multilingual prompts/responses, when converting, then output preserves content."""
        df = pd.DataFrame([
    ...
    def test_privacy_redaction_integration(self):
        """Given PII in prompts/responses, when converting, then redacted tokens are present."""
        df = pd.DataFrame([{"prompt": "My email is john@example.com", "response": "Thanks, John."}])
        with patch("ai.dataset_pipeline.clean.redact_pii_in_text_fields", return_value=(df, True)):
            chatml = convert_to_chatml(df)
            assert any("[REDACTED" in m["content"] for m in chatml)

    def test_malformed_rows_skipped(self):
        """Given rows with null/NaN or non-string, when converting, then only valid rows are included."""
        df = pd.DataFrame([
    ...
    def test_compliance_structure(self):
        """Given valid input, when converting, then output alternates user/assistant and is ChatML-compliant."""
        df = pd.DataFrame([
            {"prompt": "A", "response": "B"},
            {"prompt": "C", "response": "D"}
        ])
